...
### Allocator API support

Enable the `allocator-api2` feature to make `Arena` implement the
[`Allocator`](https://docs.rs/allocator-api2/latest/allocator_api2/alloc/trait.Allocator.html)
trait from the [`allocator-api2`](https://crates.io/crates/allocator-api2) crate.
This lets you use the arena as the backing allocator for `allocator_api2::vec::Vec`
and other collections.

On nightly, you can enable the `nightly` feature to have `Arena` implement
[`std::alloc::Allocator`](https://doc.rust-lang.org/stable/std/alloc/trait.Allocator.html),
so `HashMap` and `HashSet` (via `with_capacity_in`) can allocate into the arena.
