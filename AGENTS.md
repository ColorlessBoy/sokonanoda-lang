...
### 同一处连红 3 次 ⇒ 必须换思路或降级（2026-09-26 用户拍板）

**规则**：**同一个测试/判据连续判红 3 次 ⇒ 禁止用同一招试第 4 次** ✗。
必须**三选一**并在**同一轮内收口**：

* **(A) 降到真相层做判据** —— 端到端难判定就改在更靠近真相的那一层断言
  （front/LSP 单测），端到端留 TODO + 台账；
* **(B) 降级交付**（成本最低）—— 先保证**能保证的那一半**，把剩下那半写成
  **已知限制**进台账，并用一条"断言当前行为"的判据**防漂移**；
* **(C) 确认是真 bug 就先修根因** —— 但必须给"修前/修后"的**最小证据**
  （一条断言 + 反向验证），**不许盲试** ✗。

**为什么**：打转的成本不是那几次测试，是**没有汇报、没有提交**的几十分钟 ✗
（实测过一次：同一测试红 11 次、同一段文本手术重复 3 遍、53 分钟没提交 ✓）。
**报告节奏**：只要一轮里出现"改了没生效"的循环，就该**先汇报再动手** ✓。

> 一句话：**红 3 次就换招或降级，别把"重试"当策略。**
