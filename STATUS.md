...
- ⚠⚠ **一条"假判据"的教训（2026-09-26，如实记 ✓，比 bug 更值得记）**：编译进度的两条
  宿主判据第一版被我**插进了 runner 的循环体里** ⇒ 它们**被注册却没被执行** ✗，而收尾那行
  `tests.length - failed / tests.length` 照样把它们算进"通过" ⇒ 屏幕上显示 **36/36 passed**
  而两条测试**一次都没跑** ✗✗。**是"反向验证"抓出来的**：撤掉实现后测试**依然全绿** ⇒ 才回头
  发现它们根本没执行 ✓。⇒ 两条纪律：① **判据必须被反向验证过才算存在**（"绿"可能是假的 ✓）；
  ② **计数式收尾不能证明每一条都跑过** —— 它只数注册数 ✗（要看**每条的名字**是否出现在输出里 ✓）。
- ⚠ **subagent 纪律**（2026-09-26 恢复 ✓）：两个只读侦察并行跑（S2 编译进度接线图 / S1′ G-41
  失败模式地图），**结论都已回写台账**（G-42 的真根因 + 失败模式 A–F + 三条修法提案 ✓）。
  ⇒ **S1′ 的结论改写了 B 组的修法**：G-41 与 G-42 是**同一个洞**（闸门
  `args.len() > explicit_arity` 分不清"隐式位也写了"与"把结果函数又应用一次"）✓。
