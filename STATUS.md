...
- **进度**：**48/50** ✓（`python3 scripts/plan.py list` ✓）
- **已发布**：**`sokonanoda v0.72.0`** ✓（`gh release list` 显示 **Latest** ✓ · 2026-09-25T23:25:07Z ✓）
- **发版点**：7 个 ✓（`python3 scripts/plan.py bumps` ✓）

## ⚠ 未决（**1 项 / 2 个环节**）

- **T-D11 + T-D12** ✗ —— **第三刀：跨会话复用**（把"已验证的前缀环境"按
  `.sokonanoda/` 持久化 ✓ ⇒ **编辑中热编译**再降 ✓）+ **它的判据**
  （keystroke 基准复量 ✓ + 四件套 ✓）。
- **按用户 2026-09-26 的决定 (A)「并入后续版」** ✓ ⇒ **它们与下一个有实质内容的版本一起发** ✓
  （**D-1/D-2/D-3 的代码已在主干** ✓；**不发空版** ✓）。
- **设计入口** ✓：`docs/design/e2-plan.md` 的 **T-D11/T-D12** 两条 ✓
  （含交付物与判据 ✓）；前置结论见 **T-D7 已勾** ✓。
