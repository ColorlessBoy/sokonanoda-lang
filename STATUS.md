...
- **进度**：**62/66**（E2 50/50 收口 ✓ + **批次 N** 记法×隐式参数×产品交互 12/16 ✓ ·
  `python3 scripts/plan.py list`）
- **已发布**：**`sokonanoda v0.72.0`** ✓（`gh release list` 显示 **Latest** ✓ · 2026-09-25T23:25:07Z ✓）
- **本轮（记法专项，用户 2026-09-26）** ✓：**A1–A5 全部落地 + A0 收口** ——
  * **A1** 显示层混合形态：`ty_text` 里 ASCII `->` **227 → 0**（顺带修出 `print_back` 的
    `base` 反推对"整条带括号"是错的**潜伏 bug** + 第二趟折叠）；
  * **A2** `{a}`/`{a, b}` 折回（残留 `Set.singleton` **5 → 0**）；**A3** `{a}` 可跳转 ——
    根因是**开放练习的签名一条 hover 行都没有** ⇒ 未解出的练习里 hover/F12/高亮/引用全失效；
  * **A5/R5** 记法路径两条求解路线都补 **delta 展开兜底** ⇒ `flawed_equalities_refuted`
    从整条点名改成记法（标记 **5 → 3**，只剩 λ 体内的 `∅`：语言级缺口，Lean 同样要 `(e : T)`）；
  * **A4** prelude 可跳转 ✓：`Or`/`And`/`Iff`/`False` 的 F12 落到**前奏源文件**的真 span
    （源与喂进编译的**同一份字节**，落成真文件 = Lean 的 `Init/Prelude.lean` 同款；
    那份源作为普通文档编译干净：35 条声明 / 0 诊断 ✓）；
  * **A0** 基线 **68 → 59**（迁移 9 处）；本轮补最后一格 `SubGoal.ty`：显示副本**必须折**、
    真相字段（`suggest.rs` 回读它算建议）**一个字节都不许折**，两半各一条判据 ✓。
    **结论：59 是地板**（剩下 19 立判据 + 40 台账不做都是"别动"），再降要改记账口径 ✓。
- **真宿主 e2e**：A1/A2/A3/A4 四条 ✓（全量 **32/32**）—— 反向验证撤掉修复 ⇒
  **1 passed / 3 failed**，失败信息正是用户报的症状（混用 `->` / `Set.singleton α a` /
  F12 返回 null）✓；顺带修一条**写死夹具形状**的旧用例（取"最后一个 `sorry`"⇒ 假红）。
- **判据**：课程门禁 **36/328/99/0**（逐项不变 ✓）· front **742** / LSP **163** / CLI **25 套件** ✓ ·
  `notation-lint` **84 个文件零旧写法** ✓ · playground 锚点 exit 0 ✓ · `ci-local --fast` 全绿 ✓。
- **B0 取证（T-N9）** ✓：IA-1 三条用法实测全通（记法 `a ∈ A` · 点名短写 `Set.mem a A` ·
  `@` 全显式），且**改签名向后兼容** ✓；课程侧普查 **585** 处点名。**⛔ 结论：B2 被 B3 挡住**
  —— 库的前导类型参数改成 `{α : Type}` 会让课程门禁从 `328 checked · 0 判负` 掉到
  **`249 checked · 81 open · 7 判负`** ✗（最小复现 + 证据进缺口 **G-40** ✓）。
- **未做**：**B3（先行）→ B2**、Infoview 字号、进度展示 P1–P6。
...
- **遗留（2026-09-26 ✓，**当前挡着 push** ✗）**：`docs-lint.py` 的 **① 活文档总量**
  **已经超了**（实测 **3.02 MB > 3.00 MB**，而且 `origin/main` 上就超 ✓）——
