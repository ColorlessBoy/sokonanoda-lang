...
- **进度**：**59/65**（E2 50/50 收口 ✓ + **批次 N** 记法×隐式参数×产品交互 9/15 ✓ ·
...
- **本轮（记法专项，用户 2026-09-26）** ✓：**A1/A2/A3/A0 + A5/R5** 六条里落成五条半 ——
  ...
  * **T-N5/T-N6** 真宿主 e2e 三条（A1/A2/A3）✓ + 修一条**写死夹具形状**的旧用例
    （它取"最后一个 `sorry`"，夹具一变就静默指到别处 ⇒ 报"等 `∃` 超时"这种看不出
    真因的假红 ✗）。**反向验证**：撤掉三处修复 ⇒ **1 passed / 3 failed**，
    三条失败信息正是用户报的症状（混用 `->` / `Set.singleton α a` / F12 返回 null）✓。
    全量 e2e **32/32** ✓。
  * **A4** prelude 可跳转 ✓：`Or`/`And`/`Iff`/`False` 的 F12 现在落到**前奏源文件**里的
    真 span（源与喂进编译的**同一份字节**，落成真文件 = Lean 的 `Init/Prelude.lean`
    同款）。**实测前提销账**：那份源作为普通文档编译是干净的（35 条声明 / 0 诊断 ✓）。
    三层判据（front 同源 · LSP 读回那一行 · 真宿主 e2e）各带反向验证 ✓；
    途中抓到并修掉"**抢走被 import 模块自定义的 `Or`**"这条真回归 ✓。
  **未做**：A0 继续削基线、B2/B3、Infoview 字号、进度展示 P1–P6。
