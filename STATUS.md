...
- **T-D7 / T-D10 / T-D13** ⚠：**阶段 D 三刀的收尾**（复量 → push → CI → bump
  **0.69.0 / 0.70.0 / 0.71.0** → release ✓）—— **三刀都已实现** ✓，
  而 **D-1/D-2 的收益量不出** ✗ ⇒ **它们会是"空版"** ✗
  ⇒ ⇒ **需要你在 (A) 并入后续版 / (B) 空版 / (C) 让 D-1 真正见效 里选** ⚠（**建议 (A)** ✓）。
- **T-D11 / T-D12** ⚠：**第三刀（跨会话复用 ✓）的实现与判据** ——
  它依赖 D-1/D-2 的结论 ✓（**若走 (A) 则与后续版一起做** ✓）。

## 已完成的 45 项 ✓

**A–E 五个阶段全部收口** ✓（**B/C/D 的代码 + E 的性能门禁与文档** ✓）；
**阶段 U 全部收口** ✓（**唯一接口 + 五个面** ✓ · `docs/design/duplication-audit.md` ✓）；
**`v0.72.0` 已发布** ✓。
...
- **未决** ✓：**T-U10/T-U11**（**77 处绕过逐条收口** ✓）。
- ⚠ **五次尝试、四个夹具 bug** ✗ —— **而每一个都是从失败文本里读出来的** ✓✓：
...
- **变了什么** ✓：夹具修好 `Set` + 元数后仍红 ✗，而**失败文本前进了一格** ✓：
  现在是 **`def Weird`** ✓（**之前是 `def usesWeird`** ✓）⇒ **卡在 `Weird` 的体** ✗ ——
  `def … := True` 要求 `True` 与 `Set.subset A B` **定义相等** ✗，而 elaborate 不展开它 ✗
  ⇒ **已改用 `axiom Weird (A B : Set Nat) (P : Prop) : Set.subset A B`** ✓（**无体 ⇒ 无义务** ✓）。
- **状态** ✓：`node --check` **通过** ✓（**且这次先判语法再跑** ✓）· **重跑在 `bash-1255`** ✓。
- **未决** ✓：`bash-1255` 的结论 ✓。
- ⚠ **而"失败文本前进"是好信号** ✓：**它说明前面的错都被修掉了** ✓ ——
  这一面的调试**每一步都有可读的下一步** ✓。
