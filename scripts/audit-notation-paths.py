...
WHITELIST = {
    ...
    # ⚠ **`walk.rs` 不进白名单** ✗（round 171 实测 ✓）：`--self-test` **正是拿它当靶子** ✓
    # （"应当抓到 walk.rs 里的绕过" ✓）⇒ 把它排除 ⇒ **自检瞎掉** ✗
    # （我这一轮就这么干过 ✗，自检当场判 FAIL ✓ ⇒ 立刻撤销 ✓）。
    # ⇒ E 组那 8 条**留在基线里** ✓（它们的 ③ 结论在台账里 ✓ —— 基线多 8 条，
    #    好过**把守卫弄瞎** ✗：**守卫的可验证性 > 数字好看** ✓）。
