...
> **要一句话把活交出去？** 那段 prompt 已归档 ⇒ `docs/archive/e2-prompt-2026-09-26.md.gz` ✓。
>
...
### 6.0 **结论（2026-09-25 round 310 ✓，接手请先读这段 ✓）**

**阶段 D 的三刀全部落地/量清，而三刀都"无可赚"** ✗ —— **不是设计错了** ✓，
而是**前提过期了** ✗：计划的措辞写在 **R-3（`.sokonanoda/compiled/`，T-B5）之前** ✓，
而 **R-3 已经把"热编译"这件事做完了** ✓。

| 刀 | 状态 | 实测 |
|---|---|---|
| **D-1**（`SOKO_JUDGE_ENV_REUSE` ✓） | ✅ **已实现** ✓ · **默认关** ✓（一行可回退 ✓） | 12.8 万次判卷 ✓、命中率 **99.6%** ✓ ⇒ 两态差 **<0.3% 且方向相反** ✗ ⇒ **噪声** ✓ |
| **D-2**（`SOKO_WALK_REAL_ADD` ✓） | ✅ **已实现** ✓ · 判据**超额达成** ✓（开关态 **100 → 0** ✗✓）· **默认关** ✓ | LSP 套件两态 **34.91s vs 34.79s** ✓ ⇒ Δ **≈0.6%** ✗ ⇒ **噪声** ✓ |
| **D-3 / T-D11**（跨会话复用 ✗） | 📊 **已量证否** ✓ · **不必实现** ✗ | 小项目 **0.08s → 0.04s** ✓（热态已快 2× ✓）；整门课**压根不缓存** ✓ 且只要 **0.38s** ✓ |

**⇒ 按计划的 C3 处置** ✓（"**D 可停在任一小步**"✓）：
**0.69.0 / 0.70.0 / 0.71.0 都不发** ✗（**没有用户可见增量** ✓，**T-C7 先例** ✓："不发空的版本" ✓）
⇒ **直接进阶段 E，发 `0.72.0`** ✓ —— ⚠ **但这一条需要用户确认 (A)/(B)/(C)** ✓
（**建议 (A)：并入 E 之后一起发** ✓）。

**⇒ 但三刀留下了两样东西** ✓（**接手别丢** ✗）：
1. **两份正确但默认关的实现** ✓ —— **一行可回退** ✓，回退方式与复现命令写在代码注释里 ✓；
2. **一条守卫** ✓：`perf-gate`（阶段 E 的 T-E1 ✓）—— 它的 smoke 里含
   **`did_open_same_session`（134ms）** ✓ = **D-1/D-2 动过的那条热路径** ✓
   ⇒ **收益量不出，但"以后慢下来会被发现"** ✓✓。
