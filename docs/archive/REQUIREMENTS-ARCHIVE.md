# REQUIREMENTS 归档（历史追加日志 · 2026-09-06 → 2026-09-26）

> **这是 `REQUIREMENTS.md` §9 的逐字原文** ✓ —— 2026-09-26 文档瘦身时整体移入
> （用户要求：「文档太重了，还没实现多少东西文档先爆炸了」；设计
> `docs/design/docs-diet.md`）✓。移入前 §9 = **2989 行** ✗。
>
> **归档 ≠ 销毁** ✓：**现行要求**已提炼进 `REQUIREMENTS.md` §9（每条给归档锚点 ✓）；
> **判据与守卫**仍在 `scripts/`（`docs-lint.py` / `status-lint.py` / `gap.py` …）
> 与 `crates/**/tests/` 里**真跑** ✓；**原文可直接 grep** ✓（本文件是纯文本，
> 没有 gzip —— 它是"要求的账本"，可检索性优先 ✓）。
>
> **索引（139 条）**：
>
...
- § P-2 并行与 subagent 纪律
- § 2026-09-26 · **STATUS.md 瘦身 + lint**（用户要求 ✓）
- § 2026-09-26 · **文档瘦身 + 判据守护**（用户要求 ✓）
...
**⑲ ③ 真因锁定（2026-09-25，第 50–51 轮）· ✅ 已修（第 55 轮）**：内建的 `=` 吃掉 `=>` 里的 `=`** ✓✓✓
机械二分（同一测试内逐条加内建 ✓）：
```
...
**㉒ ③ 修法的确切位置（2026-09-25，第 54 轮）—— 下一轮动手，本轮**故意不动** ✗**
```rust
// crates/front/src/token.rs:209-220（在 `next_token` 里）
// 声明驱动的符号（第二刀）：**最长匹配优先**，且在常规分支之前——
// `𝒫`/`ᶜ`/`''`/`⁻¹'`/`×ˢ` 里前四个是标识符字符、`''` 今天根本不是合法 token，
// 只有这条路能把它们读成 `Sym`。
if let Some(symbol) = self.declared_symbol_ahead() {   // :212
    …return Token { kind: TokenKind::Sym(symbol), … };
}
match c { … }   // ← `=>` / `->` / `:=` 这些**基础多字符算符**在这里才被读成单 token
```
**修法**：`declared_symbol_ahead()` 命中时，**再问一次常规分支**——若常规分支在该位置能
读出一个**更长**的算符（例如 `=>` 长 2 > `=` 长 1 ✓），则**让常规分支赢** ✓；
否则保持现状（`𝒫`/`''`/`⁻¹'`/`×ˢ` 这些**只有**声明符号路能读 ✓ ⇒ 一条都不能少 ✓）。
**为什么本轮不动**：这一处是**词法核心** ✗ —— 规则写漏一个基础算符（`->`/`:=`/`\`/`''` …）
就会让整门课编译崩 ✗；**先把 `match c` 里的多字符算符清单读全**，再按"取更长者"改 ✓
（blast radius 太大，宁可多花一轮 ✓）。
**回归哨兵（一条都不能红 ✓）**：`cargo test -p sokonanoda-front` 全绿 ✓ ——
...
**㉔ ③ 的结构性根因（2026-09-25，第 59 轮，**用户点出**）**：**记法转化有 4 套实现、没有收口** ✗✗
用户原话：「我看到你的测试了，光标在 exists_fun 时，infoview 最上面的"目标"显示不正确，
notation 没有正确转化。请问这里是多个地方分别实现 notation 转化的吗？没有收口到同一个接口吗？」
—— **查证：他说得对** ✓✓。

**四套实现（各自为政 ✗）**：
| # | 实现 | 输入 | 干什么 | 谁在用 |
|---|---|---|---|---|
| 1 | `display::print_back` + `fold_collecting` | 文本 + **完整记法表** | **点形式 → 记法**（真正的转化 ✓） | `kernel_phase.rs:104/123/321`（`ty_text`/`val_text` ✓）、`check/mod.rs:393` |
| 2 | `semantic::tag_runs_with_notations` | 文本 + **只有符号字符串** | **只打标签/高亮**（不转化 ✗） | `query/mod.rs:488` 的 `runs(…)` ⇒ `goal_runs`/`ty_runs`/`goals_runs`（**Infoview 渲染用的就是它** ✓） |
| 3 | `display::render_expr` | AST | AST → 文本（**不转化** ✗） | `walk.rs:735`（generic open exercise 兜底 ✓）、`goals::open_goal`（**开放练习的目标** ✓，即用户这条 ✓） |
| 4 | elaborate 的 pp（`info.goal`） | 内核项 | 内核 pp 文本（**点形式** ✗） | `walk.rs:555/778`（tactic 步进目标 ✓） |

**后果（本次 bug 的形状 ✓）**：**"渲染"与"折叠"是两步** ✓，而目标生产这条链（3、4 ⇒ 2）
**只做了渲染、没做折叠** ✗ ⇒ 顶部「目标」永远是点形式 ✓；我修的 `token.rs`（在 ① 的解析里 ✓）
**碰不到**它 ✗✓ —— 这就是用户看到"还是不对"的原因 ✓。
（`AGENTS.md` 的 R-1/R-2 教训"**真相与显示是两条路，bug 活在接缝里**"在这里升级为
"**连显示自己都分了四条路**" ✗。）

**修法（收口，不是再补一处 ✓）**：定一个**唯一**的转化入口，例如
`display::render_folded(expr, &DisplayNotations) -> String`（= `render_expr` 后**必过**
`print_back` ✓），并让**所有**目标/类型/值的生产都只经它 ✓：
* `goals::open_goal` 与 `walk.rs:735` 的 `render_expr(ty)` → `render_folded` ✓（用户这条 ✓）；
* `walk.rs:555/778` 的 `info.goal` → 同样过一遍折叠 ✓（文本本就点形式 ✓）；
* `query::runs` 保持"只打标签"✓（这是**对的**分工 ✓）—— 但它的**输入文本必须先折过** ✓。
**判据（用户可见，一条就够 ✓）**：光标停在夹具 `exists_fun` 的 `sorry` 上，
**Infoview 顶部「目标」必须出现 `∃`** ✓（我新加的 e2e 用例断的就是它 ✓ ——
上一轮它红，正是因为修的是 ① 而不是 3 ✓）。

**㉕ 用户新要求（2026-09-25，原文）**：「**更新 e2-plan，设定一个完整的计划，我要求完全统一接口**」✓
⇒ 已立**阶段 U（记法转化统一接口）**：`docs/design/e2-plan.md` 的 `T-U1..T-U8` ✓。
**这是一条架构级硬要求** ✓：记法转化必须**只有一个接口**（现在**四套** ✗：`print_back` ✓ /
`semantic::tag_runs_with_notations`（只打标签 ✗）/ `render_expr`（只渲染 ✗）/ 内核 pp（点形式 ✗）），
并以 **A∖B 守卫**防第五套长出来 ✓（`scripts/audit-notation-paths.py` ✓）。
**已完成的前置**（不算替代 ✗）：`token.rs` 的"基础多字符算符更长时让路" ✓（bug ③ 的根因 ✓）。

**㉖ 用户新要求（2026-09-25，原文）**：「**另外也排查一下有没有其他模块有一样的问题：
明明是一摸一样的功能，用在不同的地方，但是我们的项目分成多个不同的实现，导致各种 bug**」✓
⇒ 这是一条**全仓审计要求** ✓（不是只修记法那一处 ✓）。
**已立进计划**：`T-U9`（审计并发起：三个只读 subagent 分头查 front / CLI·LSP·query /
scripts·编辑器 ✓）、`T-U10`（把审计结论收口成"唯一归属 + 守卫"✓）。
**样板（已知两例 ✓）**：① 记法转化四处实现 ⇒ Infoview 目标错 ✓；
② "按 `*.sokonanoda` 取课程文件"在多个脚本各写一遍 ⇒ R-3 加产物目录后
`notation-lint.py` 崩掉却不判 ✗。
**纪律**：subagent 只读、并发 3 ✓、每条结论必须带 file:line 或可复跑命令 ✓、
产出**验证后才并入** ✓、结论**回写文档** ✓（`AGENTS.md` 并行纪律 ✓）。

**㉗ T-U4 的正确形状（2026-09-25 第 64–65 轮：踩红线 + 查清）**
* ⚠ **红线**：`goal` / `binders[].ty` / `sub_goals[].ty` **同时喂 judge** ✗ ——
  在 `walk.rs` 的目标生产处加折叠 ⇒ **5 条测试判红** ✓（含 `suggest::*` 三条 ✓，
  它们靠目标文本判"`rfl` 能不能证"✓）。原话在 `kernel_phase.rs` 的注释里 ✓，
  而仓库里本就有专门证明这个分工的测试 ✓：
  `query::tests::by_step_display_is_folded_but_the_judge_input_is_not` ✓
  ⇒ **折只许折"显示副本"** ✓，判卷输入一个字节不动 ✗。（已回退 ✓ `c90fcec` ✓）
* **样板**（照它做 ✓）：`by_steps` 的显示副本就是这么干的 ✓ ——
  `compile/tests.rs:3873-3892` 断言 `goals[0].ty == "a ∧ a -> a"` ✓（折过 ✓）
  而判卷用的 AST 没动 ✓。
* **第五套实现（本轮又查到一个）** ✗：`query::notation_symbols()`（`query/mod.rs:472`）
  **扫文本**拿符号字符串 ✓（当前文件 + 各 import 模块各扫一遍 ✓）⇒ 只能"打标签"、**不能折叠** ✗；
  而且 **query 这条路够不到 `DisplayNotations`** ✗（实测 ✓）⇒ 想在 `query::runs` 里折
  是**做不到**的 ✗（不是不想做 ✓）。
* ⇒ **T-U4 的正解**：在 **front 的生产侧**给目标加一份**显示副本** ✓（照 `ty_text` 的样板 ✓：
...
**㉙ T-U4 的落点精确到行（2026-09-25 第 67 轮）—— 只差"声明级"那一段** ✓
查证 `crates/front/src/query/mod.rs`：
* **`by` 步进那条没问题** ✓：`:685-700` 的 `goal` 用的是 `ByGoalState.ty`，
  而它**本来就是折过的显示副本** ✓（`compile/tests.rs:3879` 断言 `goals[0].ty == "a ∧ a -> a"` ✓）
  ⇒ 这条不用动 ✗；
* **声明级那条是病根** ✗：`:753`（`ty_runs`）、`:771-794`（`goals_runs`）、
  `:805`（binder 的 `ty_runs`）都是拿 `d.ty` / `g` / `b.ty`（**judge 文本** ✗）去
  `self.runs(...)` 打标签 ⇒ **点形式 runs** ✗ ⇒ 扩展渲染出来就是点形式 ✓
  （用户看到的正是这个 ✓）。
**改法（一处、显示侧 ✓）**：在声明级那几处，**优先用折过的显示副本**
`DeclState.ty_text: Option<String>`（`crates/front/src/compile/report.rs:109` ✓，
注释写明"内核 pp 出来再过一遍**线 C 的记法折叠**"✓），拿不到才回退 `d.ty` ✓ ——
标签器（`semantic::tag_runs_with_notations`）只需**符号字符串** ✓，而折过的文本里
符号本来就在 ✓✓，所以这一步**不需要记法表** ✓（这正是它能在 query 层做的原因 ✓）。
`goal` 字段（judge 文本 ✗）**保持原样** ✓；开放练习的目标 ≈ 它的类型 ✓
...
它按**仓库相对**的 `target/<profile>/` 取二进制 ✗ ⇒ 当构建落在
`CARGO_TARGET_DIR`（本会话用 `/tmp/soko-target` ✓）时，stage **拷到的是仓库里那份旧构建** ✗✓
—— 于是"被测二进制"根本不是刚编的 ✓（告警里 `server 运行 0.65.0 != 扩展 v0.67.0` ✓ 就是这么来的 ✓）。
**修法（规定，两步都要 ✓）**：
1. `stage-lsp.js` **尊重 `CARGO_TARGET_DIR`** ✓（找不到就报错，别静默回退 ✗）；
2. **stage 时断言版本** ✓：读 staged LSP 的自述版本，**必须等于**仓库版本（`scripts/soko version --json` ✓），
   不等就 **exit 3** ✗ —— 这一条比第 1 条更重要 ✓：它把"测的不是当前源码"变成**一次判红** ✓，
...
**㉟ 用户新要求（2026-09-25，原文）**：「**收 T-U10（审计结论收口），并加一条全局判据**」✓
**（1）逐条处置 `scripts/notation-paths-baseline.txt` 里的 77 处绕过**，每条给出结论之一 ✓：
① **立刻迁移**到唯一接口 `DisplayNotations::render` / `render_text`；② 补一条"**必须折叠**"的判据；
③ 写进台账**不做**并说明理由 ✓。
> **硬规则（用户原话）**：**凡产出用户可见文本的（LSP / hover / 诊断消息 / 状态栏 / 项目树）
> 必须走 ① 或 ②** ✓ —— 不许用 ③ 把它糊过去 ✗。
> **已知的用户可见绕过（优先处理 ✓）**：`crates/lsp/src/lib.rs` 有 **5 处**
> （`goals.push(render_expr(…))`、`goal_text = render_expr(&goal_ty)`、
> `term_text = render_expr(body)` ✓），`crates/front/src/compile/elab.rs` 里还有
> **拼诊断消息**的绕过 ✓。

**（2）加一条面级 sweep 判据** ✓：对每个显示面（**hover / goal / 诊断 / 状态栏 / 项目树**）
各构造**含记法**的类型 ✓，断言输出里**不出现点形式** ✓；并且**必须能咬住已知反例** ——
**回退 T-U4/T-U5 的折叠时必须判红** ✓（用户原话："**咬不住的守卫等于没有**" ✓）。
...
**用户原话（要点 ✓）**：「STATUS.md 里有大量无用信息（例：『perf-gate 在跑』）」✗
—— 请把这条**落成机制**，不要写成口号 ✗。

**量到的现状** ✓：**1534 行 / 67 KB** ✗ · 含 "round N" **163 条** ✗ ·
瞬时状态词（在跑/进行中/未变/判据不变/待 CI/等 CI/⏳）**138 条** ✗ ·
3 个 `##` 段覆盖 **221–321 轮**（≈100 轮 ✗）· `docs/STATUS-ARCHIVE.md` 461 KB ✓。

**⇒ 要求（已落成判据 ✓）**：
1. **STATUS.md 只留两类内容** ✓：① 顶部「**当前快照**」（**≤ 40 行** ✗）；
   ② **最近 ≤ 3 轮**，每轮只写"**变了什么 / 现在的状态 / 未决项**" ✓，**每段 ≤ 30 行** ✗；
2. **逐轮过程与瞬时状态一律进 `docs/STATUS-ARCHIVE.md`** ✓（**或干脆不写** ✓ ——
   **CI 页面自有** ✓）；
3. **lint** ✓：**`scripts/status-lint.py`** ✓ ——
   **总行数 ≤ 200** ✗ · **禁词命中 = 0** ✗（`在跑`/`进行中`/`未变`/`判据不变`/`待 CI`/`等 CI`/`⏳` ✓）·
   **每段 ≤ 30 行**（**首段"当前快照" ≤ 40** ✓）· **净增行数 ≤ 60** ✗（与 `HEAD` 比 ✓）；
   **未决项用固定措辞"未决"** ✓（**不属禁词、可判** ✓）；
4. **反向验证** ✓：**把旧的大段贴回 `STATUS.md` ⇒ lint 必须判红** ✗（**咬不住等于没有** ✓）。
   **实测 ✓**：旧版 ⇒ **169 条判红** ✓✓。

**⇒ 验收（实测 ✓）**：**1554 行 / 67 KB ⇒ 64 行 / 4.3 KB** ✓（**-96% 行数** ✓）·
`status-lint` **exit 0** ✓ · 反向验证 **exit 1（169 条）** ✓。

### 2026-09-26 · **文档瘦身 + 判据守护**（用户要求 ✓）

**用户原话（要点 ✓）**：「文档太重了，还没实现多少东西文档先爆炸了」✗
—— 把这条**落成机制**，不要写成口号 ✗。

**量到的现状** ✓（2026-09-26 10:05 实测 ✓）：`docs/` = **8.4 MB / 419 个文件 /
105,753 行** ✗ · 根 `REQUIREMENTS.md` **258 KB** ✗ · `ROADMAP.md` 44 KB ✓ ·
`AGENTS.md` 27 KB ✓ · `STATUS.md` 12 KB（已瘦身 ✓）；最大的 12 个：
`docs/STATUS-ARCHIVE.md` 571 KB · `docs/design/vscode-editor-feedback-plan.md` 283 KB ·
`docs/notes/settheory-survey/learning-difficulties.md` 267 KB ·
`docs/design/e2-plan.md` 254 KB · `docs/design/.e2-plan.md.*.tmpdir/e2-plan.md.tmp`
254 KB（**遗留垃圾** ✗）· `docs/design/site-rebuild/research/R1-design-craft.md` 200 KB ·
`docs/notes/course-lean-style/course-inventory.md` 161 KB ·
`docs/design/course-lean-style.md` 148 KB · `docs/e2e/ledger.jsonl` 135 KB ·
`docs/CI-FAILURES.md` 127 KB · `docs/design/site-rebuild/research/R2-docs-teardown.md` 111 KB ·
`docs/TESTING.md` 104 KB。

**⇒ 诊断** ✓：文档里混了**三类**东西，要求完全不同，**绝不能一刀切** ✗：
① **活规范**（必须短、必须准）：`AGENTS.md`、`REQUIREMENTS.md` §9、`architecture.md`、
`docs/design/*.md` 的契约部分；
② **过程记录**（该归档/合并/删过程只留结论）：逐轮日记（`STATUS-ARCHIVE` 571 KB）、
CI 失败台账（127 KB）、e2e 台账（135 KB）、perf 台账、调研笔记；
③ **垃圾**（直接删）：遗留 `.tmpdir`/`.tmp`/编辑器残留。

**⇒ 要求（逐条可判 ✓）**：
1. **先盘点再动手** ✓：按三类给出清单（文件 · 大小 · 处置 = 保留/合并/归档/删除 · 判据）✓；
2. **活规范瘦身** ✓：`REQUIREMENTS.md` 只留**现行**要求（历史批次移入
   `REQUIREMENTS-ARCHIVE.md` ✓）；`docs/design/e2-plan.md` 只留**未完成/现行契约**
   （已收口环节移入归档 ✓，且 `scripts/plan.py` 必须仍可用 ✓）；
3. **过程记录只留「结论 + 指针」** ✓：逐轮日记 / 失败台账 / 性能台账保留**最近 N 条**
   （最近 30 天或最近 50 条 ✓），更早进 `docs/archive/`（可 gzip ✓）且**仍有可追溯路径** ✓；
4. **删垃圾** ✓：遗留 `.tmp` / `.tmpdir` / 编辑器残留一律删 ✓，并加 `.gitignore` 规则**防复发** ✓；
5. **立判据并接进 gate** ✓（**关键** ✗，否则一定回胖 ✗）：
   `docs/` 总量 ≤ X MB（本轮盘点后定 ✓，目标砍到 1/3 以内 ✓）·
   **单文件行数 ≤ 上限**（入口文件如 `e2-plan`/`REQUIREMENTS` 另设更严的 ≤800 行 ✓）·
   新环节的**文档预算**（设计文档 ≤150 行 ✓，只写契约不写过程 ✓）·
   `docs/**` 禁止 `.tmp`/`.tmpdir` ✓；
6. **反向验证** ✓：把砍掉的大段贴回 ⇒ 判据**必须判红** ✗（**咬不住等于没有** ✓）。

**⇒ 红线（不许动 ✓）**：**判定正确性、红线证据与可追溯性** ✗ —— **归档 ≠ 销毁** ✓：
任何"证据/判据/复现件"必须**仍能找到**（给归档路径与索引 ✓）；
**不许为了让数字变绿而删证据** ✗。

**⇒ 验收** ✓：处置**前后**的 `docs/` 体积与文件数 ✓ · 最大的 5 个文件的**去向** ✓ ·
lint **进 gate 的证据** ✓ · **反向验证判红记录** ✓。
