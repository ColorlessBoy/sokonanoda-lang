...
## 2026-09-18：把字节 offset 当字符下标（一次假缺口）

**现场**：写课程缺口复现 `G15-kernel-error-span-drift.sh` 时，我拿 `grade --json` 的
`span.start.offset` / `span.end.offset` 直接去切 Python 的 `str`（`src[:offset]`），
得到"诊断 span 跨到了下一个声明"的结论，还写进了台账（G-15）与设计文档。
复核时按**字节**切片（`src.encode()[s:e]`）才发现 span 完全精确——源文件里有 `α`/`β`
这类多字节字符，字节 offset ≠ 字符下标。

**代价**：一条不存在的缺口 + 一份断言假象的复现脚本 + 一份 300 行的 WO（WO-010 的作者
在自己复核时把它纠正成了真缺口：`query check` 的 `failed[]` 只给裸 offset、没有行列）。

**规矩**：
1. 内核/协议给的 `offset` 一律**按字节**解释（`crates/front/src/span.rs`、`token.rs` 都这么定义），
   要用行号就用 `span.start.line`，不要自己数；
2. 复现脚本里凡是"从 offset 推位置"的地方，必须写成 `src_bytes[:off]`；
3. **台账条目在写进去之前要有一次独立的复核**（这次是子代理纠正了主线 agent）；
   反过来也一样——子代理的结论（如"`{u, v}` 可以"）也要自己跑一遍再改台账。


## 2026-09-19：门禁注入的环境变量会**短路复现夹具**（台账门禁第一次跑就红）

**现场**：把「缺口即测试」接成门禁——`scripts/soko gate` 在课程门禁之后再跑
`python3 scripts/gap.py check`，并顺手把**解析到的**二进制经 `SOKONANODA_BIN` 透传
（课程门禁就是这么做的，34 个目标不必各自再解析一次）。结果 gate 第一次跑就红：
`G-11`/`G-16` 报「缺口仍在」，而我手跑同一条命令是全绿的。

**根因**：这两条复现测的**就是启动器自己的解析链**——夹具里预置一个陈旧缓存，看启动器
会不会拒绝执行。而 `SOKONANODA_BIN` 是"显式指定二进制"的逃生门，启动器**无条件采信**它，
于是夹具被绕过、陈旧缓存根本没被拒绝：复现假报旧行为，台账被判"行为回退"。
换句话说：**我为了省时间注入的那点便利，正好把被测量的东西替换掉了。**

**规矩**：
1. 门禁/CI 给复现喂环境时，只喂**被测对象之外**的东西；凡是"测解析链/测默认值/测拒绝"
   的复现，一律在**中性环境**里跑（`gap.py` 现在自己剔除 `SOKONANODA_BIN`/
   `SOKONANODA_LSP_BIN`，`gate` 也不再透传；脚本自身再加一行 `unset` 兜底）；
2. 需要覆盖的复现**自己 inline 赋值**（`SOKONANODA_BIN=… node soko …`），
   不要依赖调用者给的环境——那样"环境正确"就成了复现通过的前提；
3. 新增门禁步骤必须**在 gate 里真跑一次**再宣布落地：这条门禁的单元/自检全绿，
   接到 gate 里第一次跑才暴露问题（手跑与门禁的差异只在环境变量上）。
