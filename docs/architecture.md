...
| `builder.rs` | **新增 `EnvBuilder::hide_declars() -> DeclarMap<'a>` / `restore_declars(saved)`**（T-D8，2026-09-25）：只把 `declars` **挪走/还回**（`std::mem::take` ✓），**`dag` 不动** ⇒ **指针同一性保住** ✓（与 `with_env` 的搬来搬去**同一手法** ✓，见 `:99`/`:110` ✓）。<br>**为什么**：给"多余的 `sorry`"的**合成探针**一个"**看不见文件声明**"的环境 ✓ —— A 步把声明提前加进真 `builder` 后 ✓，探针看到它们 ⇒ **冗余判定翻转** ✗（`a_leftover_sorry_*` 那 4 条 ✓）；而**新建** `EnvBuilder` 会带来**自己的 DAG** ✗ ⇒ `def_eq` 因指针不同而**假失败** ✗。<br>**为什么是两个方法而不是闭包** ✗：调用点要同时借 `&mut self.builder` 与 `&self.known` ✓（两个**不同字段** ✓，靠**字段级借用拆分**才合法 ✓）—— 闭包会让 `self` 被**可变借两次** ✗。 | **T-D8 B 步** ✓（D-2 前缀复用）。三层回归 ✓：kernel `tests/` 3/10/5/0 ✓ · front 736/0 ✓ · CLI `--json` **逐字节相同** ✓（`a2bdf9c4fad1…` ✓，开关下也相同 ✓）。判据 ✓：默认 736/0 ✓ · **开关态 100 → 0** ✓ · 组合态 **= 基线 11** ✓。**默认路径不调用** ⇒ 判定行为**零变化** ✓ || `tests/memory_api.rs` | **新增** | 无导出文件的内存检查验收 |
...
**✅ 已自动化**（2026-09-25 ✓）：`scripts/ci-local.sh` 里那条
`gates：折叠判据的反向验证（必须判红 ✗）` ✓ —— 它把**已证明咬得住**的那两条判据在
`SOKO_NO_NOTATION_FOLD=1` 下重跑 ✓，**必须判红** ✗；**若仍然绿 ⇒ 门自己红** ✓。
（为什么需要它 ✓：本 session **三次**写出"通过但咬不住"的判据 ✗，每次都是靠**人手**反向验证才发现 ✓
⇒ 把它交给门 ✓，才不会复发 ✓。）
本仓库现成的两个干净手法：
