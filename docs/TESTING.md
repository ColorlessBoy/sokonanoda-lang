...
| 多文件 LSP（P5） | 被 import 的声明对入口可见（无诊断）；缺失 import 是诊断不是崩溃；`textDocument/definition` 能跳进被导入模块，**入口自己声明的名字仍留在入口**（项目模式不吞本地跳转）。**余项**：依赖变更后自动重编译其它打开文档未实现（`crates/lsp/src/tests/project.rs` 文件头登记） | `crates/lsp/src/tests/project.rs` :: `an_imported_module_is_visible_to_the_entry`、`a_missing_import_is_a_diagnostic_not_a_crash`、`definition_jumps_into_the_imported_module`、`definition_stays_in_the_entry_for_local_names` | `cargo test -p sokonanoda-lsp --lib project::` |
...
7. **多文件 LSP 的跨文件失效（I16 P5 余项，当前唯一开口）**：依赖文件改动后，**其它**
   已打开文档不会被自动重编译（第一版实现会在 tower-lsp 串行通知 + 客户端 socket
   缓冲下挂住，已回退并留档）。当前语义：改动的文档自己重编译；其它文档在下一次
   **自身**编辑时看到新环境。跨文件 `references`/`rename` 同样未做。补测试时从
   `crates/lsp/src/testutil.rs` 的 `did_change_at` + `wait_diagnostics_for` 起步。
