...
| 输入期补全（键入态） | 逐键输入 `funintro` 时「替换源代码」项始终在：前缀/整词/尾随空格 → 键入态项；骨架态 newText 末端为 snippet 占位符 `${0:sorry}`（接受后 sorry 选中，输入即覆盖） | `crates/lsp/src/lib.rs` :: `typed_funintro_chars_offer_the_replace_item_while_typing`、`keyword_typing_at` | `cargo test -p sokonanoda-lsp --lib` |
| 值位 `funapply` 移除 | 0.22.0 因性能移除（judge_infer 每键全前缀重编译 O(n²)）；`by` 块 tactic `apply` 不受影响；`elab-apply-*` 错误码随之删除 | `docs/design/value-keywords-v2.md` §7 | — |
