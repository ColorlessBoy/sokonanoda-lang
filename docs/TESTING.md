...
| lambda 尾关键字 | `fun … => intro / apply / by …`：lambda 体尾部识别三个值位关键字（parser-only，降低沿链下降零改动）；骨架合成 binder 避让外层 lambda 名；裸 `intro` 于非函数目标仍报 `elab-intro-not-a-function` | `crates/front/src/parser.rs` :: `parse_lambda_tail_keyword`；`crates/front/src/compile/tests.rs` :: `keywords_work_in_a_lambda_tail`、`nested_intro_in_a_lambda_tail_lowers_to_the_skeleton`、`nested_apply_in_a_lambda_tail_checks`、`nested_intro_on_a_non_function_goal_is_still_rejected`、`by_in_a_lambda_tail_*` | `cargo test -p sokonanoda-front --lib` |
...
| 值位 `intro` | `:= intro` 值位关键字的解析（`And.intro` 不受影响、带尾随名字报错）与全剥降低（声明名保留、匿名层 `x` 防撞、隐式风格保留、非函数目标 `elab-intro-not-a-function`）、零内核 Open 路径；**不替换也等价**（`intro` vs 手写骨架同判定；同页/换行/尾随空格排版无关）；LSP 补全项（`textEdit` 恰好覆盖 token、骨架文本、无 intro token 时不出现、token 词尾与同行尾随空白仍命中、跨行不命中）与 hover（展开式 + 「不替换也完全等价」措辞 + `command:sokonanoda.expandIntro` 载荷）；session 洞 span 重映射；扩展端 hover 按钮载荷解码与应用 | `crates/front/src/compile/tests.rs` :: `intro_*`、`value_intro_is_layout_independent`；`crates/front/src/parser.rs` :: `value_intro_parses_as_the_intro_keyword`、`intro_with_a_trailing_name_is_a_parse_error`、`dotted_intro_names_are_not_the_value_keyword`；`crates/front/src/session.rs` :: `session_remaps_hole_and_sub_goal_spans_on_comment_edit`；`crates/lsp/src/lib.rs` :: `completion_expands_value_intro_*`、`completion_does_not_expand_intro_*`、`intro_expansion_*`、`hover_on_value_intro_*`、`typed_intro_*`（逐字符/选中重打/折行的**真实输入序列**，用 `testutil::type_step` + `char_steps`，断言落在每个中间态）；`crates/lsp/src/inlay.rs` :: `value_intro_hole_shows_the_remaining_goal`；`crates/cli/tests/cli.rs` :: `cli_value_intro_*`；`editor/vscode/src/test/extension.test.js` :: `hover on value intro carries a clickable expand command`、`typing intro on its own line still offers the expansion` | `cargo test -p sokonanoda-front intro` && `cargo test -p sokonanoda-lsp intro` |
| 值位 `apply` | `:= apply h` 值位关键字的解析（实参 `parse_app`、括号界定范围、空实参收下交给 lowering、带点名字不受影响、不吞后续命令）与降低（复用 `judge_infer` 推断被应用名字的类型、telescope 机械与 tactic `apply` 共用 `crate::spine`、类型参数由目标实参填充、局部假设经 `open_goal` 局部覆盖层、合成洞走 `judge_terms`）；**不展开也等价**与排版无关；`elab-apply-needs-a-term` / `elab-apply-not-applicable` / 未知名字保留 `elab-unknown-identifier`；LSP 补全/hover/`expandApply` 载荷与逐字符门控；`by` 块 tactic `apply` 不受影响 | `crates/front/src/parser.rs` :: `value_apply_*`、`dotted_apply_names_are_not_the_value_keyword`；`crates/front/src/compile/tests.rs` :: `apply_*`、`value_apply_is_layout_independent`；`crates/front/src/suggest.rs`（合成洞走 judge_terms）；`crates/lsp/src/lib.rs` :: `value_apply_completion_and_hover_carry_the_skeleton`、`typed_apply_chars_only_expand_on_the_whole_word`、`apply_expansion_is_not_offered_outside_its_own_line`；`editor/vscode/src/test/extension.test.js`（V2/V3 待补） | `cargo test -p sokonanoda-front --lib` && `cargo test -p sokonanoda-lsp --lib -- apply` |
