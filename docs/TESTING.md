...
| **课程门禁（卷 I《集合论》· 教学内容层）** | 判据 **G1–G5** 与课程规模无关：G1 每目标 `grade` 退出码 0（G-10：只认退出码）；G2 目标存在（`course.json` 条目 / `lib/` / 每个画布的解答文件）；G3 解答 `exercise.open == 0` 且 `decl.checked > 0`；G4 解答覆盖画布每个具名 `exercise.open.name`；G5 `lib`+Demo 无 `sorry`。计数只进报告/台账、**从不判红**；`--selftest` 是**判据通道**的变异自检（故意坏的单元必须被判负 + 从另一个 cwd 判带 `import lib.*` 的目标 + 二分点名坏声明），`--bisect` 按顶层声明边界二分、结论不依赖诊断 span（G-15） | `courses/set-theory/tools/check.py`（唯一真相，python3、零 cargo） | `python3 courses/set-theory/tools/check.py --selftest && python3 courses/set-theory/tools/check.py`（判负时 `--only "<标签>" --bisect`；设计 `docs/design/course-gate-in-ci.md`） |
