...
| 多文件 LSP（P5） | 被 import 的声明对入口可见（无诊断）；缺失 import 是诊断不是崩溃；`textDocument/definition` 能跳进被导入模块。**余项**：依赖变更后自动重编译其它打开文档未实现（`crates/lsp/src/tests/project.rs` 文件头登记） | `crates/lsp/src/tests/project.rs` :: `an_imported_module_is_visible_to_the_entry`、`a_missing_import_is_a_diagnostic_not_a_crash`、`definition_jumps_into_the_imported_module` | `cargo test -p sokonanoda-lsp --lib project::` |
