...
| 服务器自述 | `soko/version` 返回 `{version, pid}`，供扩展的重启命令在重启前后各问一次——旧 pid 消失 + 新 pid 出现 + 版本变化，把「旧进程退出、新进程是新版」变成可见事实（扩展更新后跑旧服务器是最常见的困惑） | `crates/lsp/src/lib.rs` :: `Backend::version`；`crates/lsp/src/tests.rs` :: `version_request_reports_version_and_pid`；`editor/vscode/extension.js` :: `restartServer` 的重启前/后回执 | `cargo test -p sokonanoda-lsp --lib -- version_request` |
...
- **LSP 结构债（0.56.0）：`lib.rs` 4256 → 3988（删重复）→ 1105 行**（把两个测试模块
  移出文件 + 抽出 `protocol.rs` 159 行 wire 类型、`tokens.rs` 107 行 semantic token
  辅助）。测试断言零改动（214+6 条 assert 与移出前逐行等价），117/117 保持全绿。
  测试文件位置见 §1 的 LSP 行；`tests.rs` 2567 行的拆分方案见 `docs/HANDOVER.md` §4。
