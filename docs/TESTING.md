...
| 值位 `intro` | `:= intro` 值位关键字的解析（`And.intro` 不受影响、带尾随名字报错）与全剥降低（声明名保留、匿名层 `x` 防撞、隐式风格保留、非函数目标 `elab-intro-not-a-function`）、零内核 Open 路径；LSP 补全项（`textEdit` 恰好覆盖 token、骨架文本、无 intro token 时不出现）与 inlay（intro 后显示剩余目标）；session 洞 span 重映射 | `crates/front/src/compile/tests.rs` :: `intro_*`；`crates/front/src/parser.rs` :: `value_intro_parses_as_the_intro_keyword`、`intro_with_a_trailing_name_is_a_parse_error`、`dotted_intro_names_are_not_the_value_keyword`；`crates/front/src/session.rs` :: `session_remaps_hole_and_sub_goal_spans_on_comment_edit`；`crates/lsp/src/lib.rs` :: `completion_expands_value_intro_*`、`completion_does_not_expand_intro_*`；`crates/lsp/src/inlay.rs` :: `value_intro_hole_shows_the_remaining_goal`；`crates/cli/tests/cli.rs` :: `cli_value_intro_*` | `cargo test -p sokonanoda-front intro` && `cargo test -p sokonanoda-lsp intro` |
