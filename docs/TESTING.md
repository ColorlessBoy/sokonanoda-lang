...
| 输入期补全（键入态） | 逐键输入关键字时「替换源代码」项始终在：前缀/整词/尾随空格 → 键入态项（textEdit 覆盖前缀、newText = 关键字全词）；实参输入 → 非空列表；整串完成 → 骨架态项（σ 实例化骨架）。v1 整词门控被推翻 | `crates/lsp/src/lib.rs` :: `typed_funintro_chars_offer_the_replace_item_while_typing`、`typed_funapply_chars_offer_the_replace_item_then_the_skeleton`、`typed_funapply_in_a_lambda_tail_keeps_the_suggestion_at_every_keystroke`、`keyword_typing_at` | `cargo test -p sokonanoda-lsp --lib` |
| 关键字组合 | `funintro (funapply X)`：`parse_atom` 原子位识别、`lower_at` 复用（目标 = 剥完后的最终目标）、context 贯穿（外层链 + 剥出层）；组合端到端（σ 实例化前提 + 合并骨架）、等价性契约、错误面沿用既有码 | `crates/front/src/parser.rs` :: `keywords_parse_inside_parentheses_enabling_composition`；`crates/front/src/compile/tests.rs` :: `funintro_with_a_nested_funapply_lowers_the_composition`、`funintro_composition_*`、`nested_funintro_without_a_function_goal_is_still_rejected` | `cargo test -p sokonanoda-front --lib` |
| lambda 尾关键字 | `fun … => funintro / funapply / by …`：lambda 体尾部识别三个值位关键字（parser-only，降低沿链下降零改动）；骨架合成 binder 避让外层 lambda 名；裸 `funintro` 于非函数目标仍报 `elab-intro-not-a-function` | `crates/front/src/parser.rs` :: `parse_lambda_tail_keyword`；`crates/front/src/compile/tests.rs` :: `keywords_work_in_a_lambda_tail`、`nested_funintro_in_a_lambda_tail_lowers_to_the_skeleton`、`nested_funapply_in_a_lambda_tail_checks`、`nested_funintro_on_a_non_function_goal_is_still_rejected`、`by_in_a_lambda_tail_*` | `cargo test -p sokonanoda-front --lib` |
...
| 值位 `funintro` | `:= funintro` 值位关键字的解析（`And.intro` 不受影响、带尾随名字报错）与全剥降低（声明名保留、匿名层 `x` 防撞、隐式风格保留、非函数目标 `elab-intro-not-a-function`）、零内核 Open 路径；**不替换也等价**（`funintro` vs 手写骨架同判定；同页/换行/尾随空格排版无关）；LSP 补全项（`textEdit` 恰好覆盖 token、骨架文本、无 funintro token 时不出现、token 词尾与同行尾随空白仍命中、跨行不命中）与 hover（展开式 + 「不替换也完全等价」措辞 + `command:sokonanoda.expandIntro` 载荷）；session 洞 span 重映射；扩展端 hover 按钮载荷解码与应用 | `crates/front/src/compile/tests.rs` :: `funintro_*`、`value_funintro_is_layout_independent`；`crates/front/src/parser.rs` :: `value_funintro_parses_as_the_funintro_keyword`、`funintro_with_a_trailing_name_is_a_parse_error`、`dotted_funintro_names_are_not_the_value_keyword`；`crates/front/src/session.rs` :: `session_remaps_hole_and_sub_goal_spans_on_comment_edit`；`crates/lsp/src/lib.rs` :: `completion_expands_value_funintro_*`、`completion_does_not_expand_funintro_*`、`funintro_expansion_*`、`hover_on_value_funintro_*`、`typed_funintro_*`（逐字符/选中重打/折行的**真实输入序列**，用 `testutil::type_step` + `char_steps`，断言落在每个中间态）；`crates/lsp/src/inlay.rs` :: `value_funintro_hole_shows_the_remaining_goal`；`crates/cli/tests/cli.rs` :: `cli_value_funintro_*`；`editor/vscode/src/test/extension.test.js` :: `hover on value funintro carries a clickable expand command`、`typing funintro on its own line still offers the expansion` | `cargo test -p sokonanoda-front funintro` && `cargo test -p sokonanoda-lsp funintro` |
| 值位 `funapply` | `:= funapply h` 值位关键字的解析（实参 `parse_app`、括号界定范围、空实参收下交给 lowering、带点名字不受影响、不吞后续命令）与降低（复用 `judge_infer` 推断被应用名字的类型、telescope 机械与 tactic `apply` 共用 `crate::spine`、类型参数由目标实参填充、局部假设经 `open_goal` 局部覆盖层、合成洞走 `judge_terms`）；**不展开也等价**与排版无关；`elab-apply-needs-a-term` / `elab-apply-not-applicable` / 未知名字保留 `elab-unknown-identifier`；LSP 补全/hover/`expandApply` 载荷与逐字符门控；`by` 块 tactic `apply` 不受影响 | `crates/front/src/parser.rs` :: `value_funapply_*`、`dotted_funapply_names_are_not_the_value_keyword`；`crates/front/src/compile/tests.rs` :: `funapply_*`、`value_funapply_is_layout_independent`；`crates/front/src/suggest.rs`（合成洞走 judge_terms）；`crates/lsp/src/lib.rs` :: `value_funapply_completion_and_hover_carry_the_skeleton`、`typed_funapply_chars_only_expand_on_the_whole_word`、`funapply_expansion_is_not_offered_outside_its_own_line`；`editor/vscode/src/test/extension.test.js`（V2/V3 待补） | `cargo test -p sokonanoda-front --lib` && `cargo test -p sokonanoda-lsp --lib -- funapply` |
| 服务器自述 | `soko/version` 返回 `{version, pid}`，供扩展的重启命令在重启前后各问一次——旧 pid 消失 + 新 pid 出现 + 版本变化，把「旧进程退出、新进程是新版」变成可见事实（扩展更新后跑旧服务器是最常见的困惑） | `crates/lsp/src/lib.rs` :: `Backend::version`、`version_request_reports_version_and_pid`；`editor/vscode/extension.js` :: `restartServer` 的重启前/后回执 | `cargo test -p sokonanoda-lsp --lib -- version_request` |
| 声明级 binder | `theorem f (a : A) (h : B a) : C := v` 的解析/降级（Forall 类型 + Lambda 值）、`:= sorry` 直出 codomain 目标与上下文、无 fun 闭合、`:= funintro` 只剥剩余、`by` 初始上下文、宇宙参数/隐式 binder 消歧、无类型 binder 教学报错 | `crates/front/src/parser.rs` :: `decl_binders_*`、`untyped_decl_binder_*`、`universe_params_before_*`、`example_with_decl_binders_*`；`crates/front/src/compile/tests.rs` :: `decl_binders_*`；`crates/cli/tests/cli.rs` :: `cli_decl_binders_*`、`cli_untyped_decl_binder_*`；`crates/lsp/src/inlay.rs` :: `decl_binder_hole_shows_the_codomain_goal`；`crates/lsp/src/lib.rs` :: `completion_expands_intro_after_declaration_binders_to_the_residual` | `cargo test -p sokonanoda-front decl_binder` && `cargo test -p sokonanoda-cli decl_binder` |
