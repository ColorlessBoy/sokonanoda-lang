...
| 启动器**行为**契约（H1） | `scripts/soko update` 的强制刷新**没写成缓存就不得报成功**：空缓存 + `SOKONANODA_OFFLINE=1` + 可用回退（`SOKONANODA_BIN`）时仍须 exit `3` + stderr `cache NOT refreshed` + `download:` 原因 + stdout 仍报实际用的回退；完全无解（无回退、无网络）须 exit `3` + 可行动消息（`could not provide matching binaries`）而**不是 TypeError 崩栈**；`setup` 只承诺就绪，允许静默回退（exit 0） | `crates/cli/tests/launcher.rs` :: `update_never_reports_success_when_the_cache_was_not_refreshed`、`an_unresolvable_environment_fails_with_the_actionable_message_not_a_crash`、`setup_may_resolve_a_fallback_without_failing` | `cargo test -p sokonanoda-cli --test launcher`（**真跑 Node**；CI 缺 `node` = 硬失败，本地缺则打印 skip——见 `dsh.rs` 只断言文件形状，覆盖不到行为） |
...
- **总量（2026-09-17 第九十一轮续，`cargo test --workspace --locked`，全绿）**：
  **772** 个测试 / 0 failed / 6 ignored（本轮 `redundant-sorry` 新增 8 条 + 摘掉 2 条
  `#[ignore]`）— kernel 43 + arena 1（需 `LEAN_KERNEL_ARENA`）+ memory_api 8 = 52；
  front lib **414** + perf 3 = 417；cli 单测 5 + 集成 **180**（cli 81 / course 6 /
  course_status 4 / dsh 8 / examples 1 / extension 32 / launcher 3 / opencode 8 /
  protocol 10 / **query 13** / skill 4 / watch 10）= 185；lsp lib **118**；
  doc-tests 6 ignored（内核既有）。测试目标共 19 个（cli 集成目标 12 个）。
