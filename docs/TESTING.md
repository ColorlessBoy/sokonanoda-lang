...
| **记法编辑器导航（0.65.3；线 D 收口，缺口 G-23 关账）** | 记法符号在编辑器里是**一等目标**：① `definition` 跳到**声明它的模块**（`QueryDoc::notation_at` + `module_path`，跨 `import` 也算）；② hover 给"展开成什么" + **原始类型**（内核 `judge_type_of_constant`）三态（本文件声明 / 内建 / `import` 来的）；③ hover 的 `range` **只覆盖符号本身**（`symbol_span_at`，与 `symbol_at` 共用同一次词法查找）；④ 记法**不是名字** ⇒ `rename` 被拒、`documentHighlight` 不误命中**外层 binder**（binder 的 span 覆盖整段类型标注）；⑤ `ResolvedTarget::Notation` 的 `resolution` 在报告装配后**不被回填覆写**（装配只碰 `Declaration`）；⑥ `Expr::Notation.symbol_span`（T-D14）是"光标是否压在符号上"的 AST 侧答案 | front：`crates/front/src/parser.rs` :: `a_notation_nodes_symbol_span_covers_only_the_symbol`；`crates/front/src/compile/tests.rs` :: `a_notation_hover_row_keeps_its_notation_resolution_after_assembly`、`notation_records_a_hover_row_covering_the_whole_notation`；`crates/front/src/references.rs` :: `resolve_at_does_not_mistake_a_notation_symbol_for_the_enclosing_binder`；`crates/front/src/query/tests.rs` :: `notation_folding_does_not_clobber_a_use_points_resolution`；LSP：`crates/lsp/src/tests/navigation.rs` :: `goto_definition_on_a_notation_symbol_lands_on_its_declaration`、`a_notation_symbol_does_not_resolve_to_the_enclosing_binder`；`crates/lsp/src/tests/hover.rs` :: `hover_on_a_{locally_declared,builtin,imported}_notation_symbol_*`、`a_notation_hover_range_covers_only_the_symbol`；e2e：矩阵 #7/#8 | `cargo test -p sokonanoda-lsp --lib notation` && `SOKO_VSCODE_TEST_VERSION=1.138.0 scripts/vscode-e2e.sh --grep "notation symbol"` |
