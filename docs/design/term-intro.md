...
> **已废弃（0.27.0，2026-09-14）**：值位关键字 `funintro`（本文件的「值位
> `intro`」）已整体移除，见 `docs/design/remove-funintro.md`。本文仅存档历史
> 设计；by 块 tactic `intro` 不受影响。
...
- 课程：在 unit4-by-tactics 加「两种写法对照」一节并配练习——
...
### 12.4 追加（0.18.0）：可选答案 = 前端隐式替换

用户反馈「必须强制展开，不能直接等价成对应的 fun 展开片段」——症结是裸
`intro` 的洞就是 `intro` token，而 `:= intro <任何东西>` 原本是语法错误，
学习者**没有任何途径**往那个洞里写答案，唯一出路就是接受展开。

修法（不改内核）：`intro` 接受**可选答案**——`intro <expr>` 由前端隐式替换成
`fun … => <expr>`（`compile/intro.rs` 的 `peel_all_pi` 末端放答案而非洞）。
答案必须是**最终目标**的证明；判定仍由内核终审（错答案 → `kernel-rejected`，
与手写完全一致）。有答案时没有合成洞、也不携带编辑器骨架（没有可展开的
东西了）。实参语法与 `apply` 对称（`parse_app`，复合形状加括号）。

同轮修正 hover 展开按钮的载荷形状：VS Code 命令链接的实参是 **JSON 数组**
