...
> **入口文件不进冻结** ✓（用户 2026-09-26 要求「**修判据而不是改冻结值**」✗）：它们由判据 ③
> （≤800 行）管 —— ③ 抓的是"变成巨石" ✗；而冻结在"**计划随新批次长大**"时**必然误红** ✗
> （实测：另一会话加「批次 N」+138 行 ⇒ 撞死冻结值 ⇒ **真阻塞**：**所有人**的推送都被拦 ✗）。
> ⇒ **③ 管入口，④ 只管非入口** ✓；非入口确需长大 ⇒ `--freeze` 重新基线或手改 JSON ✓。
>
