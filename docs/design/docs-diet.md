...
> **逃生门用过两次** ✓（都评审可见）：① 文档预算写进 `AGENTS.md` 时它 368 → 375 行 ⇒ 手改
> JSON 抬那一条 ✓；② **入口文件**（`e2-plan` 这类**计划**会随新批次长大 ✓）⇒ 冻结上限取
> **800**（= 判据 ③ 的入口上限 ✓），**不是**当前行数 —— 但**已收口的批次必须移入归档** ✗。
> 其余条目一律冻结在**当前行数** ✓（放宽必须手改 JSON ⇒ 评审可见 ✓，不在脚本里抬阈值 ✗）。
>
