...
- [ ] `T-N14` **B3**（**先行**：B2 的前置 · **半落**）—— **第一刀已落 ✓**：① **零显式实参的常量**（`∅` = 裸 `Set.empty`，设计 §8 的 X14 推迟档）**已修** ✓（判据 + 反向验证 ✓，缺口 G-40 收口）；**剩下两档**：② 陪域是 `Set` 的函数体 ③ **记法路径**套隐式 binder 的库定义 ⇒ 真库实测 `328 checked · 0 判负` 掉到 `226 · 14 判负` ✗（缺口 **G-41**）；另登记 **G-42**（查签名表用裸名 ⇒ `namespace` 里裸名引用不触发；显然修法会回归 `#check some Nat` ✗）
