...
**做**：**`documentHighlight`（文件内）** ✓
光标落在记法符号上时，给出**这个符号在本文里的每一处**（词法：喂符号表再收
`Sym(symbol)` token 的 span —— **不用子串匹配**，否则注释/字符串里的同形字符也会算进来）。
`lib/LSP` 侧只把 span 映射成 `Range`；扫描本身在**前端** `notation_input::symbol_occurrences`
（词法器与内建符号表都归前端所有）。

> **T-D30 的意图一条都不破**：先前的守卫是"光标在符号上 ⇒ 直接返回空"，
> 但它**真正**要防的是"把外层 binder 点亮"（binder 的 span 覆盖整段类型标注
> `(h : a ∈ A)`，旧回退会把 `∈` 解析成 `h`）。现在改成给符号自己的每一处，
> 判据也改成语义化的那条：**每一段被点亮的文本都必须就是符号本身**（不是"必须是空"）。

**不做（给理由）**：
* **`references`（跨文件）** ✗ —— LSP 的 `references` 是**项目级**语义，而记法符号的
  使用点要扫**闭包里每个模块**；那等于每次请求都把每个文件词法扫一遍 ✗，
  与本项目"性能是生命线"的取向冲突。**文件内**的需求已由 `documentHighlight`
  覆盖（同一个问题换个更便宜的问法）✓。
* **`rename`** ✗ —— 符号是**源级糖**：它的"名字"是记法**声明行里的字符串字面量**，
  而且同一个符号可能在**多个模块**里各声明一次（见 T-D23）⇒ 重命名必须同时决定
  "要不要改别的模块那条声明"、"要不要改声明里的字符串" ✗。
  真正自洽的功能是"重命名**目标常量**"（那属于标识符重命名，是另一件事）。
...
提示得出来，跟"这个文件 import 了谁"无关。

**决定**：**导航跟作用域，输入提示不跟 —— 两者的偏差是有意的**。
1. **导航**（`definition`/`hover`）**以作用域为准**：闭包记法表（import 进来的）
   + 本文件声明 + 内建。三者都没有 ⇒ **返回 `null`**，**不许**因为"词法上认得出
   它是个符号"就编一个定义出来；
2. **输入提示**保持全局（不动）——用户敲 `\in` 时想要的是"怎么打这个符号"，
   不是"这个文件里能不能用"。

**判据**：`crates/lsp/src/tests/navigation.rs::a_notation_symbol_out_of_scope_is_not_resolved`
—— 建一个真临时项目（`SetLib` 声明 `∈`、入口**不** import 它），断言
`textDocument/definition` 返回 `null`。

## 跨文件记法的"定义"落在哪（T-D21 的落地决定，2026-09-24）## 跨文件记法的"定义"落在哪（T-D21 的落地决定，2026-09-24）
...
| # | 未做项 | 理由（不是"没时间"） |
|---|---|---|
| 1 | ~~**源码级 print-back**（goal/hover 显示 `∃ x, …` / `{a}` / `Aᶜ` 而不是点名形式）~~ **⛔ 本行的理由已作废（2026-09-21）；方案已立项为线 C（T-C10…T-C41），权威设计 `docs/design/notation-aware-printing.md`** | ~~**内核冻结下销不掉**~~ **作废**：类型文本由**冻结内核的 pp** 产出（硬规则 1：`crates/kernel/**` 一个字节不许动），而记法**不进内核**——内核只看见 `Exists α (fun …)`。要做得二选一：① 改内核 pp（违反硬规则 1）；② 在 elab 保留"源 → 核"的映射、把所有 pp 消费点（goal 面板、hover、错误文本、`#check`、`by` 回读）换成源级渲染——那是**两套真相**，且回读路径（§11.9）会立刻分叉。收益（面板好看一点）不抵成本与风险。**任务明确要求这一项不做**，故记在这里。**⛔ 2026-09-21 作废**：① "内核冻结"这条前提已失效（用户同日解冻内核）；② 而且**根本不需要动内核**——实测内核的记法打印是**死代码**（`ExportFile.notations` 全仓库无一处 insert），`pp_expr` 又同时是 `#check`/`#reduce` 的出口（动它就动 `--json` 字节）。正解是 **front 侧的显示边界重写**（零内核改动、`DisplayText` 护栏让"回读"编译不过），设计见 `docs/design/notation-aware-printing.md` §3，计划见 §6 线 C |
