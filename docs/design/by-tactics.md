...
>   其中 **`misses=247 ≈ 9.5s = 全文件的 77%** ✓（`docs/perf/ledger.jsonl`）。
> * **试过并否决的路**（都有实测依据 ✓）：
>   **K1-a**（纯 front 的 `TrustPlan` 复用）—— 机制正确 ✓、两态 `--json` 逐字节相同 ✓，
>   但**命中数 = 0** ✗（贵的那 89/247 次 miss 全在**冷开**路径上，那里前缀确实无人
>   担保）⇒ **零收益** ✗；
>   **K1-b**（共享内核环境）—— 内核侧原语已就位 ✓（`EnvBuilder::with_env` ✓、
