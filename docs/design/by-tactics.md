...
**为什么不能缓存环境**（先说清楚哪条路是死的，免得下一刀又去试）：
最自然的修法是"把前缀的已判定环境留下来复用"，但内核 API 不允许——
`EnvBuilder` 字段私有、`new(arena, config)` 是唯一构造入口，`finish(self)` 消费
自身；`ExportFile` 只读（`Env` 是带 cutoff 的只读视图），且 **`crates/kernel/` 是
冻结快照（硬规则 1）**。名字下标（`NamePtr::decl_idx`）绑定在构建它的 builder 上，
拿新 builder 造出来的 `Declar` 去查旧环境会查错槽位。所以"oleans 式复用"要等
内核侧开口子，不是前端能自己做的。
