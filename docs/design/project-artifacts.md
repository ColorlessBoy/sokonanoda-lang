...
* **保留策略 = 每个入口只留最新一条**（`project::cache::update_index`，索引在
  `meta.json` 的 `entries` 里：`入口路径 → 条目键`）：同一个入口的新结果**替换**它的旧
  条目 ⇒ 目录大小**天然有界**（= 入口文件数），且不会淘汰"还在用的"条目。
  ⚠ **这里踩过一次**：最初的"32 条上限 + mtime 淘汰最旧"在课程门禁（反复判 ~35 个文件）
  下会**互相淘汰刚写下的条目** ⇒ 命中率崩掉、反复重编 ⇒ CI 的 `test` job 从 ~15 分钟
  变成 50+ 分钟 ✗（判据 `every_entry_keeps_its_own_artifact_round_after_round`：
  34 个入口，条数上限版本实测只剩 **32 ≠ 34** ⇒ 红；索引式 ⇒ 34 条全在、第二轮全命中 ✓）。
  **现实形状的复核**（真课程切片：真 `lib/**` 9 个 + 3 个真单元，`build <dir>` 连跑三轮）：
  第一轮 `0 hit, 12 compiled` ⇒ 第二轮 **`12 hit`** ⇒ 第三轮 **`12 hit`** ✓
  （条目=入口文件数、不再互相淘汰；交叉验证"稳态命中"这一面，判别性的那一面由上面
  34 入口的判据给）。
* 提供 `--clean`（§3.7）；
...
### 8b. **上限策略的实测反例（2026-09-25，CI 变慢）** ✗→待修
阶段 B 收尾那次 CI 的 `test` job 从 ~15 分钟变成 50+ 分钟（**不是卡住**：日志显示
"Workspace tests" 早已跑完、后续步骤在推进 ✗ 我一开始误判成 hang 并取消了两次）。
最可能的原因就是本节的上限策略：**课程门禁**会反复判 `courses/set-theory` 的 ~35 个文件，
而"32 条按 mtime 淘汰"会在这一轮里**互相淘汰**刚写下的条目 ⇒ 命中率崩掉 ⇒ 反复重编 ✗。

**修法（下一轮做，已想清语义）**：把"条数上限"换成 **"每个入口文件只留最新一条"**
—— 产物目录的语义本来就是"每个入口当前的编译结果"，它**天然有界**（= 入口文件数），
而且不会淘汰"还在用的"条目。实现要一条**索引**（`meta.json` 里记
`{"<入口路径>": "<key>"}`，写新条目时按索引删掉同一入口的旧条目），
避免为了找旧条目而去读 MB 量级的条目本体。判据：同一门课连判两轮，第二轮
**命中率不降**（数字进台账），且目录条目数 ≤ 入口文件数。
