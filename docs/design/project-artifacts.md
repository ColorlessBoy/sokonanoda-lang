...
### 1.3 `--clean` 现状：**全局**
`build --clean` → `cache::clean()` → 删 `<root>/compiled/*.json`
...
* 体积：项目条目是**整份 `ProjectReport`**（含事件流）——真实课程单条几十 KB 量级
  （T-B6 量一次写进台账）；不做上限，写进"未决"（§6）；
* 提供 `--clean`（§3.7）；
...
| 环节 | 改什么 |
|---|---|
| **T-B5**（CLI） | `front::project::cache` 的三个入口显式带上模块根：`load_at(root, digest, options)` / `store_at(root, digest, options, project)` / `clean_at(root)`；CLI 侧 `build.rs` 传 `plan.root`（**现成**：`project/mod.rs:136`）；`overlay` 非空 ⇒ 只写全局（§2 的不变量） |
