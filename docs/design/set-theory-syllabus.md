...
### 4.1 调研底稿：Mathlib 记法的逐字取证（2026-09-18 第二轮）

> **来源**：见 `docs/notes/settheory-survey/prior-art-report.md` §1.3。
> 三条会改我们设计的事实：
...
### 4.2 as-built：课程**一律用记法**，点名形式退为"底下站着什么"（2026-09-21）

> **这一节原先写的是"记法分十步慢慢引入、今天先写点名形式"。G-04 落地 + 全课程
> Lean 化之后反过来了**：卷 I 与入门课的代码**一律用记法**。点名形式只剩两处正当
> 用途——① **课程库自己的定义体**（`Set.mem` 的定义里不能用 `∈`，那是自指；
> `Set.union` 的定义体同理）；② **记法对照页** `units/notation-cheatsheet.sokonanoda`
> （**故意**并列两种写法，是教学装置不是残留）。
> 下表因此改读成「**记法 → 它底下的点名形式**」，供讲解时展开用。

| 记法 | 底下的点名形式（讲解时展开） | 出现在 |
|---|---|---|
| `x ∈ A` | `Set.mem α x A`（= `A x`） | 单元 1 |
| `A ⊆ B` | `Set.subset α A B` | 单元 2 |
| `∅` | `Set.empty α` | 单元 2 |
| `{a}` / `{a,b}` | `Set.singleton α a` / `Set.pair α a b` | 单元 2 |
...
> 连成熟项目都这么选，我们照抄。
>
> 教学上仍然保留"同一命题两种写法"那一页（记法对照页）——只是它现在是**复习/解释**用
> （"这个符号底下站着什么"），不再是"记法还没解锁时的临时替身"。
...
| 想 `inductive And` | **G-02** | **已修**（0.59.0 起）：直接 `inductive And` 并派生 `And.rec`；入门课仍保留 axiom 族作为**教学装置**（C2.5 待拍板） |
| 想 `inductive Exists` | **G-03** | **已修**：卷 I 的 `lib/Exists.sokonanoda` 就是真归纳 + `binder_notation "∃"` |
| 想写 `∈`/`⊆` | **G-04** | **已修**：直接写记法（符号由 `lib/Set` 声明、随 `import` 传播）；点名形式只在库的定义体与记法对照页里出现 |
