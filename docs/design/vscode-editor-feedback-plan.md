...
> **✅ 收口（2026-09-24，第 72 轮）：勾掉这一条，但两件事必须跟着走 ——**
> **(1) 它的性能判据没达成** ✗（命中 0、两态耗时在噪声内 ✓，证据在
> `docs/perf/ledger.jsonl`）；**(2) 因此本条的 `⬆ bump minor` 顺延到 T-K12c** ✗
> —— 不给一次**零收益**的改动发版 ✓。勾它的依据是 §0.3 的完成定义里那一条
> **"性能无退化"** ✓（已满足：开关默认关不掉行为、两态 `--json` 逐字节相同 ✓），
> 以及"机制 + 诚实测量 + 文档"三件都已交付 ✓。**它不是一次提速，是一次
> 有判据的阴性实验** ✓ —— 而它把靶心指向了 T-K12（judge_infer 的 247 次 miss
> = 9.5s/12.4s ✓）。
>
...
>   **⛔ b 的验收结果（2026-09-24）：不通过 ✗ —— 影子与内核阶段**不等价**。**
>   对照判据（`SOKO_SHADOW_CHECK=1`，失败表按 `cmd` 同键、并去重）：
>   * `unit01-sets-membership`：影子 `failed=[27,28,30]` vs 内核 `failed=[]` ✗
>     —— **影子把内核接受的声明判成失败** ✗✗（最危险的方向 ✓）；
>   * `unit12-solution`：两边各有对方没有的项（影子多 28/29/31/38/39/41/63/70/
>     78/80/82/91，内核多 53/56/58/67/69/73/81）✗。
>
>   ⇒ **按事先写下的规则：在 b 停下，不进 c** ✓（把"可见前缀与真实环境分叉"
>   这种会改判定的风险挡在 judge 之外 ✓）。**这是这次拆分最大的价值** ✓。
>
>   **🔎 第 73 轮的新证据与线索**：
>   * **`skip` 不是原因** ✗：把已知失败集镜像进影子后（`Walk.shadow_skip` ✓，
>     重放时整条跳过 ✓），两个文件的对照结果**一模一样** ✗ ⇒ 这两个是**冷编译**、
>     `skip` 为空 ✓ ⇒ 分叉在别处 ✓。
>   * **新线索（具体可查）** ✓：`unit01-sets-membership.sokonanoda` 自己只有
>     **10 条**顶层声明 ✓，而影子的 `decls=72` ⇒ 影子把 **import 闭包的 lib**
>     也算进去了 ✓，失败的 cmd **27/28/30 正落在 lib 段** ✓ ⇒ 分叉极可能在
>     **闭包/import 维度**（内核阶段对"依赖模块的声明"与"本单元声明"的处理不同 ✗）。
>     **下一步就从这里查** ✓（别再猜 ✓）。
>
>   **🎯 第 74 轮：根因方向锁定（最强证据）** ✓
>   给观测加上**错误原文**后，影子失败的三条给出的都是
>   **`def_eq failed: def_eq mismatch`** ✗（不是 "unknown const" ✗）：
>   ```
>   Set.notMem_empty@27: Rejected("def_eq failed: def_eq mismatch
>     expected: Pi (α : Sort(1)), Pi (a : $0), (Not.[] (((Set.mem.[] $1) $0) (Set.empty.[] $1)))
>     actual:   Pi (α : Sort(1)), …")
>   Set.mem_univ@28 / Set.mem_singleton_self@30 同形
>   ```
>   ⇒ **声明是同一条**（都来自 `ops` ✓），**差的是环境** ✓：影子环境里
>   `Set.mem` / `Set.empty` / `Not` 的**定义或宇宙层级**与内核阶段不一致 ✗
>   （注意 expected 里的 `$0`/`$1` 层级 ✓）。
>   **下一步（具体、可判定）**：把**影子环境**与**内核环境**里同一个符号
>   （先取 `Set.mem`）的定义/类型**打出来逐字比** ✗ —— 两边差在哪一行，
>   分叉就在哪 ✓。**别再往下猜** ✓。
>
>   **💡 第 75 轮：根因确认（一条命令即可复现）** ✓✓
>   对照"内核阶段**实际检查**了几条"与"影子往环境里加了几条"：
>   ```
>   $ sokonanoda --json courses/set-theory/units/unit01-sets-membership.sokonanoda
>   内核事件：decl.checked = 2（demo_subset_def, demo_mem_def）· exercise.open = 6
>   影子：decls = 72（含 import 闭包里 lib 的全部声明）
>   ```
>   ⇒ **内核阶段不重查依赖单元（lib）的声明** ✗（它们已被信任 ✓），而**影子把
>   整个闭包的 `ops` 全部重放并重查** ✗ ⇒ lib 声明被"**再次加入**"（不同的
>   `decl_idx` ✗）⇒ 后面依赖它们的声明报 **`def_eq mismatch`** ✓✓
>   —— 与第 74 轮抓到的错误原文（符号同名、层级 `$0`/`$1` 对不上）**完全吻合** ✓。
>
>   **⇒ 修法（具体、可实现）**：影子的重放**必须尊重闭包的信任边界** ✓ ——
>   只重放**本轮 pass 真正会检查**的那些命令（即跳过已被信任的依赖单元前缀 ✓，
>   对应 `TrustPlan.before` / 单元边界 / `skip` ✓），而不是无脑重放整个 `ops` ✓。
>   **验收仍是同一条**：`SOKO_SHADOW_CHECK=1` 的对照判据变成 **`一致=true`** ✓。
>   **这条线索价值很高** ✓：它把"影子不等价"从"不知道差在哪"变成了
>   "**差在信任边界没镜像**" ✓ —— 一个明确的、可以一次改对的点 ✓。
>
>   **尚未查清的分叉来源（历史记录）**：候选按可能性排序 ——
>   (1) **`skip`/`trust` 语义**：增量路径下内核阶段会**跳过**已知失败/已信任的命令
>   （`prefix_failures` / `TrustPlan`），我的重放**无条件重查** ✗ ⇒ 两边失败表
>   天然不同 ✓；(2) **`OpenExercise`**：内核阶段对它做的是**签名探针**
>   （`ByIndex(env_before)`，`kernel_phase.rs:159-164`），我完全没镜像 ✓；
>   (3) **pass1/pass2**：内核可能跑两趟、第二趟带着 pass1 的失败集 ✗。
>   **建议**：先把 (1)(2) 逐个对齐再谈 (3)；**在对照判据变成 `一致=true` 之前，
>   T-K12c 不开工** ✓。
>
>   **📦 存档决定（2026-09-24，第 72 轮）**：分叉来源基本锁定 —— 内核阶段在
>   **增量记账**下会**跳过**已信任（`TrustPlan.before`）与已知失败（`skip` /
>   `prefix_failures`）的命令 ✗，而影子**无条件重查** ⇒ **影子偏严** ✓
>   （`unit01` 内核 0 失败、影子 3 失败，正合此解释 ✓）；再叠加 pass1/pass2 的
>   合并语义 ✓。要对齐就得把**整套增量记账**也镜像一遍 ✗ ⇒ 那是**实质重构**，
>   不是"小环节" ✓。**故按备选路线：把 T-K12 标成"需要更大重构"存档** ✓，
>   先把 §13 里**不碰判定**的条目推完 ✓。
>
>   **代码处置** ✓：影子环境已**关进 `SOKO_SHADOW_CHECK` 开关**（默认不建 ⇒
>   热路径**零成本** ✓；打开才复现对照实验 ✓）；T-K12a 的两个内核侧成果
>   （`EnvBuilder::with_env` ✓ + `install_all_preludes` 唯一实现 ✓）**保留** ✓
>   —— 它们本身正确、有判据、且是将来那次重构的基座 ✓。
>
>   **备选路线（若 b 长期对不齐）**：K1-b 的收益（9.5s）很诱人，但它要求"影子
>   与内核阶段**逐条等价**"；若这条做不到，就转去 §13 里**不碰判定**的条目
>   （线 L 的课程/文档项、CP 检查点、T-K20′/T-K21 的诊断项 ✓），
>   把 T-K12 标成"**需要更大重构**"存档 ✓。
>
>   ~~**下一步（b 的验收）**：把这两个数与**内核阶段**的结果对照~~
>   **（已完成，结果见上 ✗）** —— `failed=27`
...
##### T-K12c 影子环境接进 judge（⏸ 存档·不做）

**最终结论（2026-09-24，第 76 轮）**：**不做** ✓，理由是**机制级**的、不是"没时间" ✗：
1. `decl_idx` 是**挂在被 intern 的 `NameNode` 上的全局可变槽位** ✓（`builder.rs:244`
   写、`env.rs` 读且不校验 ✓，见 `docs/design/by-prefix-reuse.md` §2.2）；
2. 影子在**独立环境**里重放同一批 `Declar` ✓ ⇒ 会**改写同一批槽位** ✗；
3. 影子一旦**跳过**任何一条声明（实测它失败 3 条 ✗）⇒ 后续索引**整体错位** ✗
   ⇒ 依赖它们的声明报 **`def_eq mismatch`** ✓✓（与观测完全一致：从 cmd 27 起、
   集中在 import 闭包的 lib 段 ✓）。
⇒ 要让影子等价，就得**同时**镜像信任边界**并**消除 `decl_idx` 的全局耦合 ✗
—— 后者是**内核级重构**（K1-c 当初被否的同一堵墙 ✓），远超"小环节" ✓。
**收益（9.5s）依然真实** ✓，但它属于"**需要内核级重构**"那一档 ✓，
和 T-K13（K1-c 备选）应当**一起重新设计** ✓，而不是继续在 front 侧绕 ✗。

**保留物** ✓：影子 + 对照判据（`SOKO_SHADOW_CHECK=1` ✓）**保留**，作为将来那次
重构的**验收工具** ✓ —— 它现在能一条命令回答"新环境与旧环境是否逐条等价" ✓。

##### T-K12c 影子环境接进 judge（历史规格）

**状态**：前半（T-K12a）已完成 ✓；后半**存档** ✓，原因与全部证据见上面的
T-K12 各段（尤其"⛔ b 的验收结果：不通过 ✗"与"📦 存档决定"）。
**开工前提**：`SOKO_SHADOW_CHECK=1` 的对照判据必须变成 **`一致=true`** ✓
（即影子失败表与内核阶段**逐条相等**）—— 在那之前不要动 judge ✗。
**要镜像的东西**（按可能性排序）：`skip` / `prefix_failures`（已知失败跳过）、
`TrustPlan.before`（已信任跳过）、pass1/pass2 的失败集合并 ✓。
**收益仍在那里** ✓：`judge_infer` 的 247 次 miss = **9.5s / 12.4s**（见
`docs/perf/ledger.jsonl`），一旦等价成立，它就是单次改动里最大的一刀 ✓。

##### T-K13 **K1-c（备选）：`EnvBuilder::snapshot()` 克隆式检查点**

> **⛔ 最终结论（2026-09-24，第 79 轮）：内核侧已做完 ✓，但接线撞上同一堵墙 ⇒ 整条 K1 线一并存档 ✗。**
>
> **看穿的墙**：`decl_idx` 是**"每个名字全局唯一"的槽位** ✗（`builder.rs:244` 写、
> `env.rs` 读且不校验 ✓）⇒ **两个环境无法共存** ✗：无论把 `add_declar` 做进哪个
> builder ✓，都会改写**共享**槽位 ⇒ 让**另一个**环境的名字→下标映射失效 ✗。
> 这一条同时解释了 T-K12c 的全部现象 ✓（跳过一条 ⇒ 整体错位 ⇒ 假 `def_eq mismatch` ✓）。
>
> **推论**：
> * `snapshot()` **安全** ✓ 正因为它**只读**（不 `add_declar` ✓）；
> * 但要让 judge 用上快照 ✓，**主 builder 在 judge 那一刻就必须已含前缀** ✗
>   —— 而 judge 跑在 walk 里、kernel 阶段**之后**才 add ✗ ⇒ 唯一出路是
>   **把 check-then-add 搬进 walk**（walk 边 elaborate 边检查、kernel 阶段不再重查 ✓）。
>   那就是那个"**结构性重构**" ✓：`snapshot`/`with_env` 让它**安全** ✓，但它**不小** ✓
>   （要动 `finish_pass` 的 PendingOp 消费顺序 + 两遍检查的合并语义 ✗）。
>
> **⇒ 处置**：整条 K1 线（T-K12c 剩余 + T-K13 接线）**一并存档** ✗，
> 归入"**需要内核级重构：把 check-then-add 移进 walk**"这一档 ✓。
> **收益记录在案** ✓：`judge_infer` 的 247 次 miss = **9.5s / 12.4s**（见
> `docs/perf/ledger.jsonl`）—— 将来做那次重构时，靶心与验收工具都现成 ✓
> （`SOKO_JUDGE_STATS` ✓ + `SOKO_SHADOW_CHECK` ✓ + `kernel-check.sh` 五步 ✓）。
> **本次调查的净产出** ✓：三个可复用原语（`with_env` ✓ / `snapshot` ✓ / 六个
> interner 与 `Dag` 的 `Clone` ✓）+ 一套能一条命令判定"新环境是否与旧环境逐条
> 等价"的对照工具 ✓ + 一份把墙指到具体代码行的诊断 ✓。
...
>   **⛔ 但不是"纯机械"（2026-09-24 第 85 轮更正）** ✗ —— 会碰到**语义**：
>   `build_one` 的成败判定是（`build.rs:132-139`）：
>   ```rust
>   let ok = project.entry_module().is_none_or(|m| m.events.errors.is_empty())
>         && !project.has_errors();
>   ```
>   ⇒ 今天是"**以该文件为入口**"编的 ✓，所以"这个文件好不好"看的是**它自己当入口**
>   的结果 ✓。分组后组内其他文件只能看**共享那次编译** ⇒ `project.has_errors()`
>   是**全项目**的 ✗ ⇒ **一个模块报错会把同组文件全判 `failed`** ✗✗
>   —— **`build.summary` 语义就变了** ✗（判据明确要求"语义不变" ✓）。
>   **可行的对齐方式** ✓：分派时用**归属到该文件模块**的诊断 ✓
>   （`ProjectReport.diagnostics` 每条带 `module` 归属 ✓，`report.rs:130` ✓）
>   ⇒ `ok(file)` := "没有归属到该模块的诊断" ✓ —— 但**这与"以它为入口编一次"
>   是否等价，必须先证明** ✗（可能不等价：入口视角会多/少看见某些诊断 ✓）。
>   **⛔⛔ 更根本的问题（2026-09-24 第 86 轮，实验前先读 API 就发现了）** ✗：
>   `plan_project(entry_path, …)` 的文档第一行是「解析项目根、**加载闭包**（不编译）」
>   （`project/mod.rs:193-200` ✓）⇒ 它加载的是**该入口的闭包** ✗（只含它 `import`
>   的那些模块 ✓），**不是整个模块根下的所有文件** ✗。
>   ⇒ 同一模块根下**互不 import** 的两个文件 = **两个不同的闭包** ✗ ⇒
>   "按模块根分组、每模块只编一次"**在现有 API 下做不到** ✗：
>   编了组内一个 plan ✗ ⇒ 另一个文件**根本不在 `ProjectReport.modules` 里** ✗
>   ⇒ 没有东西可以"分派" ✓✗。
>   **⇒ T-K30 的真实体量**：需要一个**新的 front API** ——
>   "把一个模块根下的**全部文件**作为**一个 unit 集**编一次，并按文件给出报告" ✗
>   （底层机制是有的 ✓：`units: &[SourceUnit]` 本来就支持多单元闭包 ✓；
>   但 `plan_project` 只吃**一个入口** ✗ ⇒ 要新增规划入口 ✓）。
>   再加上前面那条 `ok` 语义问题 ✗ ⇒ **T-K30 应重新定级为"需要新 API + 语义论证"** ✓，
>   不是 CLI 编排的小改 ✗。
>
>   **⇒ 所以 T-K30 需要先做一次"语义等价性"论证/实验** ✓（现在还要先加 API ✓）。
>   **另一条已注意到的线索** ✗：`build.rs:112-113` 的注释写着"闭包 v1 **不进缓存**"
>   但代码里**确实**在 `cache::load`/`store`（`:126`/`:137`）✓ ⇒ 注释过期 ✓，
>   顺手核一下缓存语义是否有意为之 ✓。
>
...
## 14. 内核级重构档：把 check-then-add 移进 walk（**⛔ 已被 E2 计划取代，2026-09-24**）

> **交接注意** ✗：本节是**历史**（E1 阶段的思考过程与刹车点记录 ✓）。
> **当前计划是 `docs/design/e2-plan.md`** ✓ —— 它把这里的内容重新分成了
> 5 个阶段 38 个环节 ✓（阶段 D 就是"把 check-then-add 移进 walk" ✓，
> 但拆成 3 个小步、3 次发版 ✓，且每步都能发布 ✓）。
> **要开工请看 `docs/E2-HANDOVER.md`** ✓。


> **为什么单列**：§13 的 124 条已全部勾完 ✓，但**性能主线（生命线）的真正收益
> 还没拿到** ✗。病灶与靶心都已量清 ✓：冷开 `unit12-solution` 的 `judge_infer`
> **misses=247 ≈ 9.5s = 全文件 77%**（`docs/perf/ledger.jsonl`）✓；而 §13 里所有
> "在现有架构内捡便宜"的候选都**实测否决** ✗（K1-a 零收益 ✓、T-K31 无收益 ✓、
> T-K30 需新 API ✓、K1-b 接线撞墙 ✓）。
>
> **那堵墙（一句话）** ✗：`decl_idx` 是"**每个名字全局唯一**"的槽位
> （`builder.rs:244` 写、`env.rs` 读且不校验 ✓）⇒ **两个环境无法共存** ✗
> ⇒ 要让 judge 看见"已验证的前缀"，**只能让 walk 自己的环境就是那个环境** ✓
> ⇒ 也就是**把 check-then-add 从 `kernel_phase` 移进 walk** ✓。
>
> **为什么现在敢做** ✓（§13 的净产出让风险可控）：
> * `EnvBuilder::with_env`（借出装回 ✓）与 `snapshot()`（只读副本 ✓）已就位 ✓，
>   各有判据 ✓；
> * **`SOKO_SHADOW_CHECK=1` 的对照器** ✓ 能一条命令回答"新环境与旧环境是否
>   **逐条等价**" ✓ —— 这正是那次重构最需要的验收器 ✓；
> * `scripts/kernel-check.sh` 五步 ✓ + 课程门禁计数 ✓ + 全语料 `--json` 逐字节 ✓
>   三件套现成 ✓。

### ⛔ R0 的结论（2026-09-24，第 95 轮）：**影子这条"彩排"路走不通** ✗

**做了什么** ✓：把影子修到与内核阶段等价的四次尝试 —— ① 镜像 `skip`（已知失败集 ✓）；
② 加**趟号**观测（排除"跨趟口径"✓）；③ 镜像**归纳块的 check-then-add + 首个失败 break**
（`kernel_phase.rs:313-325` ✓）。

**结果** ✗：**三个假设全部被实测否证** —— 每一次改完，`unit01` 的对照都是
`影子 [27,28,30] vs 内核 []` ✓、错误原文始终是
`Set.notMem_empty / Set.mem_univ / Set.mem_singleton_self` 上的
**`def_eq mismatch`**（不是 `unknown const` ✓）。

**⇒ 结论** ✗：影子环境与内核阶段的分叉**不是**这三处记账造成的 ✓，而是更深的东西
（`def_idx`/`decl_idx` 的分配与 `def_eq` 的比较口径 ✓ —— 恰恰是"把 check-then-add
移进 walk"那次重构要**动**的地方 ✓）。**用影子做彩排，等于要求先理解整个
check-then-add 流程** ✗ ⇒ **它不是一条便宜的彩排路** ✓。
**所以 R0 判为"不可便宜达成"** ✗，按刹车点**就地停下** ✓（风险未外溢 ✓）。

> **📌 本批（线 K 收口，34 个提交）的 bump 判断（2026-09-24，第 96 轮）** ✓：
> **不发版** ✗。理由（按 §0.2 的口径 ✓）：bump 点标的是**性能收益** ✓
> （"36.1s → 个位数秒"这类 ✓），而本批的实际内容是**惰性内核 API**
> （`with_env` / `snapshot` / interner+`Dag` 的 `Clone` ✓ —— **生产路径零调用** ✓）
> 加**文档与台账** ✓ ⇒ **用户可见行为零变化** ✗ ⇒ 没有可发的收益 ✓。
> **0.65.5 仍是当前线上版本** ✓（`gh release list` 已核对 ✓）。
> 将来那次重构（R1–R3）若兑现 9.5s ✓，**再按 minor 发版并闭环核对** ✓。

**⇒ 对 R1–R3 的建议（交用户定夺）** ✓：
* 那次重构（R1–R3）**技术上仍然可行** ✓，但**失去了彩排器** ✗ ⇒ 只能直接做，
  安全网是**三件套**：`scripts/kernel-check.sh` 五步 ✓ + 课程门禁计数**逐项不变** ✓ +
  全语料两态 `--json` **逐字节相同** ✓ + 事件计数不变 ✓；
* **代价与风险** ✗：要动 `finish_pass` 的 PendingOp 消费顺序（check-then-add 的
  **两遍语义**）✓ —— 属于"改判定路径"✗，必须多轮、每轮全量验证 ✓；
* **收益** ✓：冷开 `unit12-solution` 的 `judge_infer` miss ≈ **9.5s = 77%** ✓
  （12.4s → 目标 ~3s ✓）。
* **我的建议** ✓：**除非你愿意为这条线投入多轮**（每轮都要跑五步 + 全语料对拍 ✓），
  否则**先不动** ✗ —— 现状（0.65.5 ✓）是稳的 ✓，而这 9.5s 只在
  "**大文件的 `by` 密集解答**"这一形态上出现 ✓（`unit08` 的判定只占 7% ✓，
  见 T-K03 的 profile ✓）。
...
| 阶段 | 做什么 | 验收（判据） |
|---|---|---|
| **R0** | **先把影子修到等价** ✗：补上 §13 里没镜像的三样 —— `skip`/`prefix_failures`（已知失败跳过 ✓，已做）、**`TrustPlan.before`**（已信任前缀跳过 ✗ 未做）、**pass1/pass2 失败集合并** ✗ 未做；再加"闭包/单元边界" ✓ | `SOKO_SHADOW_CHECK=1` 的对照判据变成 **`一致=true`** ✓（`unit01` 与 `unit12-solution` 都要 ✓） |
| **R1** | **把 check-then-add 移进 walk**：walk 边 elaborate 边 `with_env` 检查 ✓、过则 `add_declar`、失败进失败表 ✓；`kernel_phase` 改为**消费 walk 的结果**（不再重查 ✓），语义（跳过/信任/两遍）逐条对齐 | `kernel-check.sh` 五步全绿 ✓ + 课程门禁计数**逐项不变** ✓ + 全语料两态 `--json` **逐字节相同** ✓ + **事件计数不变** ✓ |
