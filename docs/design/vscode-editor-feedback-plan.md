...
- [ ] `T-K12c` **K1-b 后半：影子环境接进 judge**（⏸ **存档**：对照判据判定影子与内核阶段**不等价** ✗ —— 差在增量记账 `skip`/`trust`/pass1-pass2 ⇒ 需要**实质重构**；影子已关进 `SOKO_SHADOW_CHECK`，默认零成本 ✓；本条的 `⬆ bump minor` 随之顺延 ✓）
