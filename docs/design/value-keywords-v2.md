...
> **已废弃（0.27.0，2026-09-14）**：本设计描述的两个值位关键字均已移除
> （`funapply` 0.22.0、`funintro` 0.27.0，见 `docs/design/remove-funintro.md`）。
> 本文仅存档历史决策；值位现在只保留普通表达式与 `by` 块。

> 状态：**已实现并发布（0.21.0，2026-09-13）**——S1 改名 / S2 输入期补全 /
> S3 组合全部落地（实施记录见各 commit 与 CHANGELOG）。
> 原文：设计定稿（2026-09-13，第四十三轮）。用户三需求，本文是唯一权威口径；
