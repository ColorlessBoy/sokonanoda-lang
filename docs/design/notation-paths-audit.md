...
## 1.5 为什么基线停在 **59**（T-N16 的结论）

89 → 68 → **59** 之后，**剩下的 59 已经不是"待办"，而是"两类别动"**：

* **立判据 19 条** —— 产物是**用户可见文本** ✓，但那条路**同时是判定输入**
  （`suggest.rs` 回读洞的期望类型、`elab.rs` 回读声明签名）⇒ 折了就是**改判定** ✗。
  处置不是迁移，而是**立判据钉住"必须不折"**（T-N16 已补齐最后一格 ✓）；
* **台账不做 40 条** —— 判定输入 / 内部探针 / 测试夹具，**没有**用户可见文本 ✓；
  其中 `walk.rs` 的 8 条是 `--self-test` 的**靶子**，把它们加白名单会让自检**瞎掉** ✗
  （round 171 实测）⇒ **守卫的可验证性 > 数字好看** ✓。

⇒ **要让这个数字继续下降，唯一的正当途径是改"记账口径"**（例如给这 59 处加
**行内声明**，让基线只留"没声明的"）—— 那是**守卫的重新设计**，不是"再迁几处" ✓。
在那之前，**59 是地板**：任何**新增**绕过仍然会被拦下 ✓（ratchet 照常工作 ✓）。
...
| `goals.rs` **9 条**（`:440/:513/:701/:702/:1033/:1051/:1178/:1288/:1484/:1540`） | `SubGoal.ty` ⇒ wire（`query/mod.rs:857-866`）+ LSP hover（`lsp/lib.rs:1698-1706`「此处 `sorry` 的期望类型」） | ✅ **2026-09-26 补齐（T-N16）**，**两半分开钉**：**显示副本**（wire 那份克隆 + hover）**必须折** ⇒ `crates/cli/tests/notation_fold.rs::hole_expected_types_fold_on_the_display_copy`（开关两态必须不同、关掉回到 ASCII）+ **反向验证** ✓；**真相字段**（`DeclState.sub_goals[].ty`）**一个字节都不许折** ⇒ `crates/front/src/compile/tests.rs::hole_expected_type_is_judge_input_and_stays_raw`（**直接读字段** —— `grade --json` 的「两态逐字节相同」在「建议**按需**算」下**碰不到**它 ⇒ 那是咬不住的守卫 ✗）+ **反向验证** ✓ |
