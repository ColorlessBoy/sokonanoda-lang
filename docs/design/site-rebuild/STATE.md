...
设计层已定稿（方向、令牌、字体、规则手册、审计），基础设施已就位（截图工具、
导航单源、卫生检查、字体管线、走查数据）。**缺的是样式表三件套与 21 个页面**。
...
| `scripts/check-site.py` | 卫生检查，7 条新断言（链接/版本/导航/元数据/体积/无位图/无内联样式/turnstile） |
...
```bash
python3 scripts/site-verify.py          # 13 项，含浏览器审计
python3 scripts/site-verify.py --quick  # 12 项，跳过浏览器
python3 scripts/site-verify.py --json   # 机器可读
```

它把已有的三个工具（`check-site.py` / `gen-site-nav.py --check` /
`site-audit.py`）与**七条新判据**合成一份报告，分两组：
...
**K10 零伪造（数据保真）**、K11 出处可查、**K12 课程计数可复现**、K3/K4 渲染审计。
...
- **K10 零伪造**：把页面里当作"真实内核输出"渲染的片段抽出来，回
  `site/data/*.json` 里逐条找。找不到 = 编的或数据过期。当前 **91 条录制值全部回查通过**。
  这是本站最重要的一条正确性检查——站点上所有内核输出都是预计算的，
  要么逐字来自数据文件，要么就是编的，没有第三种可能，而人眼分辨不出来。
- **K12 课程计数可复现**：站点唯一一类"数字来自一次测量而非某个文件"的内容。
  判据不是"现场重跑一遍"，而是 **HEAD 的课程 + 已发布二进制**能不能跑出页面写的数
  （`git archive HEAD courses/set-theory` → 临时目录 → 钉住的 0.61.0 跑门禁）。

**当前结果：13/13 全绿，28/28 页，exit 0。**
...
用 `git archive HEAD` 把 HEAD 的课程解到临时目录、配已发布的 0.61.0 二进制重跑，
得到 **36/329/99/0**，与站点**逐项相同**。所以：

- 站点发布的是**已发布版本**的计数，**是对的**；
- 差异 100% 来自工作树未提交的课程改动；
- 判据必须是"HEAD + 发布二进制"，不能是"现场跑一遍"——否则每次并行开发都会误报。

这条也顺带确认了 §5 的版本钉纪律：**站点的一切事实以 HEAD + 发布产物为准**。
...
所以站点上每一行 JavaScript 都可以是坏的，而 15 项全绿通过。
