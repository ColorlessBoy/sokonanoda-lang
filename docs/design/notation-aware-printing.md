...
### 3.3e as-built：边界与"折过的子树被应用"（T-C25，2026-09-21）

**边界：命中不了就回退点名，不猜。** 折叠层只做**二元 infix 族**；其余形态一律
原样。四种都取课程库里的**真实写法**（`courses/set-theory/lib/Set.sokonanoda`）：

| 形态 | 写法 | 折叠 |
|---|---|---|
| 一元前缀 | `prefix:100 " 𝒫 " => Set.powerset` | ✗ 回退（操作数在 `rhs`） |
| 一元后缀 | `postfix:100 " ᶜ " => Set.compl` | ✗ 回退（操作数在 `lhs`） |
| 零元常量 | `notation "∅" => Set.empty` | ✗ 回退 |
| binder 位 | `binder_notation "∃" => Exists` | ✗ 回退（第 2 个操作数是 lambda） |
| **元数对不上** | `Set.mem α a`（3 元只给 2 个） | ✗ 回退 |

回退是**设计**不是缺陷：这些形态的操作数位各不相同，规则要各写一套；**猜错的
代价**（把 `Set.powerset α A` 打成 `𝒫 A`，丢掉 `α`）比不折坏得多。

**顺带修掉一个真 bug：折过的子树被应用时，就地替换会改变语义。**

`(Set.mem α a A) B` 里 `Set.mem α a A` 是**完整的三元应用**，就地换成 `a ∈ A`
得到 `a ∈ A B`——它**重新解析成 `Set.mem α a (A B)`**，换了个意思。这是
"逐段替换"相比"整棵树重渲染"唯一丢掉的保证（§3.3c 选前者是为了保住 binder 写法
与 `Type 0`）。

**规则**：折过的子树如果**被应用**（父节点还是 `App`），把替换范围**上提到这条
应用脊的根**，整条脊交给 `render_expr`——括号归它管。实测：

```
(Set.mem α a A) B   →  (a ∈ A) B      （不是 `a ∈ A B`）
Set.mem α a A -> P  →  a ∈ A -> P     （父节点是 Arrow，不是应用 ⇒ 就地换安全）
```

**这里踩到一个隐蔽的排序坑**：折出来的记法节点**取被折那段的 span**，而"上提到
脊根"的那一处**起点与它相同**（`Set.union α (Set.union α A B) C` 的两处替换都从 0
开始）。只按起点**稳定**排序会让**内层排在前面**，随后"取最外层"规则反而把真正的
