...
(function () {
  var TEXT = {
    zh:
      ...
      "   cd sokonanoda-lang\n" +
      "\n" +
      "2. 读 AGENTS.md。按它的 Setup 准备环境：先跑 sokonanoda setup，再跑 sokonanoda doctor --json。如果 PATH 里还没有 sokonanoda，就照 README 的下载示例取与仓库版本一致的 release 二进制（版本锁定，禁用 releases/latest；不用 cargo；不碰官方 Lean 工具链）。\n" +
      "\n" +
      "3. 按 skills/sokonanoda-teacher/SKILL.md 当老师：在 playground.sokonanoda 上出带 sorry 的练习，练习前挂 2-3 行 \"-- soko:hint ...\" 提示阶梯。\n" +
      "\n" +
      "4. 判卷只认内核：跑 sokonanoda --json playground.sokonanoda，逐行读 JSON 事件（禁止文本比对）。讲解与反馈用简体中文。\n" +
      "\n" +
    ...
    en:
      ...
      "   cd sokonanoda-lang\n" +
      "\n" +
      "2. Read AGENTS.md. Prepare the environment as its Setup section says: run sokonanoda setup, then sokonanoda doctor --json. If sokonanoda is not on PATH yet, download the release binary pinned to the repository version exactly as the README example does (never releases/latest; no cargo; no official Lean toolchain).\n" +
      "\n" +
      "3. Follow skills/sokonanoda-teacher/SKILL.md and teach me: add sorry exercises to playground.sokonanoda, each preceded by a 2-3 line \"-- soko:hint ...\" ladder.\n" +
      "\n" +
      "4. Grade only through the kernel: run sokonanoda --json playground.sokonanoda and read the JSON events line by line (never compare text). Explain in English.\n" +
      "\n" +
